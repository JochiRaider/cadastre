...
| `permitted_production_uses` | Yes unless deterministically blocked | `[]`; must be non-empty when `deterministic_block_code` is null | Canonical set from `SourceDatasetProductionUseTokenSet`. Duplicate values fail. A value grants only the named production use and does not grant source authority, absence, cleanup, retraction, graph expiry, watermark, or API authorized-negative output by itself. |
...
#### SourceDatasetProductionUseTokenSet

`SourceDatasetProductionUseTokenSet` is the closed token set for `SourceDatasetCatalogRow.permitted_production_uses`. The field is a canonical set: input duplicates fail validation, canonical serialization sorts values lexically, unknown values fail before row activation, and omission materializes to `[]` before checksum computation.

| Token | Permitted effect | Forbidden interpretation |
| --- | --- | --- |
| `raw_import` | The selected row may be used by `020` feed read and raw import when all feed, package, validation, and manifest refs validate. | Does not permit silver output, gold output, absence-sensitive effects, graph output, analysis output, or API authorized-negative output. |
| `silver_observation` | The selected row may be used by `050.NormalizeObservation` to emit `CadastreSilverObservation` when mapping refs validate. | Does not permit gold output, source authority, absence, cleanup, retraction, graph expiry, or watermark behavior. |
| `positive_gold_candidate` | The selected row may be considered as positive evidence input to `080.DeriveFacts` for predicates listed in `supported_fact_predicate_refs`. | Does not permit absence, cleanup, retraction, graph expiry, watermark, no-change proof, or compliance negative output. |
| `resolver_input_only` | The selected row may produce resolver input only through `070` after resolver artifact and manifest gates pass. | Does not permit direct gold, membership, nonmembership, deletion, cleanup, graph expiry, graph edge removal, or canonical merge by itself. |
| `supporting_evidence_only` | The selected row may be preserved as supporting evidence for later owner evaluation. | Does not permit facts, no-change proof, negative output, cleanup, retraction, graph expiry, watermark, or API authorized-negative output. |
| `validation_only` | The row may appear only in validation fixtures, shadow/canary validation, or blocked supporting material. | Does not permit production read/import or production mutation. |
| `deterministically_blocked` | The row is an exact deterministic block row for the named blocked scope. | Does not permit any production mutation; a block row still requires validation refs, mutation-prohibition refs, package-set refs when package-supplied, and manifest refs. |

A non-block `SourceDatasetCatalogRow` must have a non-empty `permitted_production_uses` set. A deterministic block row must either set `permitted_production_uses = [deterministically_blocked]` or set it to `[deterministically_blocked, validation_only]` when the block participates only in validation. Any other production-use token on a deterministic block row fails validation before activation.
...
| Public `source_dataset` token | Required row presence | Required `permitted_production_uses` | Gold predicate support | Default absence-sensitive posture | Deterministic block code when blocked | Required `060` closure refs when active | Package-set requirement when package-supplied | Validation row family |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `endpoint_inventory` | Active catalog row required when selected. | `raw_import`, `silver_observation`, `positive_gold_candidate`. | Must match `MVPSourceDatasetPredicateSupportClosure` row for the same token. | Positive-only unless exact `060` rows authorize the requested effect. | `SOURCE_DATASET_CATALOG_ROW_MISSING` when omitted. | Required for absence, cleanup, retraction, graph expiry, or watermark. | Required. | `120-SOURCE-DATASET-CATALOG-*`; `120-FEED-CLOSURE-*`; `120-SOURCE-CLOSURE-*`. |
| `configuration_inventory` | Active catalog row required when selected. | `raw_import`, `silver_observation`, `positive_gold_candidate`. | Must match `MVPSourceDatasetPredicateSupportClosure` row for the same token. | Positive-only unless exact `060` rows authorize the requested effect. | `SOURCE_DATASET_CATALOG_ROW_MISSING` when omitted. | Required for configuration absence, cleanup, or watermark. | Required. | `120-SOURCE-DATASET-CATALOG-*`; `120-FEED-CLOSURE-*`; `120-SOURCE-CLOSURE-*`. |
| `vulnerability_scan` | Active catalog row required when selected. | `raw_import`, `silver_observation`, `positive_gold_candidate`. | Must match `MVPSourceDatasetPredicateSupportClosure` row for the same token. | Coverage-sensitive; fixed or absent output requires exact `060` closure. | `SOURCE_DATASET_CATALOG_ROW_MISSING` when omitted. | Required for vulnerability absence, fixed state, cleanup, retraction, or watermark. | Required. | `120-SOURCE-DATASET-CATALOG-*`; `120-SOURCE-CLOSURE-*`; `120-COVERAGE-DOMAIN-*`. |
| `control_evaluation` | Active catalog row required when selected. | `raw_import`, `silver_observation`, `positive_gold_candidate`. | Must match `MVPSourceDatasetPredicateSupportClosure` row for the same token. | Control pass/fail/unknown requires exact control-result mapping and source closure. | `SOURCE_DATASET_CATALOG_ROW_MISSING` when omitted. | Required for pass, fail, unknown, not-checked, not-applicable, absence, or watermark. | Required. | `120-SOURCE-DATASET-CATALOG-*`; `120-SOURCE-CLOSURE-*`; `120-ERROR-REGISTRY-*`. |
...
## MVP Activation Catalog Materialization Contract

This contract makes the feed read gate executable for the selected MVP scope. A production feed read, raw import, mapping activation, source-authority closure evaluation, graph handoff, analysis handoff, API filtering, package-supplied catalog selection, replay, or validation acceptance must resolve the selected activation-catalog material before output.

Concrete row bytes and checksums are supporting material. They are not core prose. The public spec defines row schemas, row-set identities, default block behavior, validation requirements, package gates, and manifest requirements. Product governance must supply concrete row bytes, selected row refs, row checksums, row-set checksums, validation fixture checksums, expected output or error checksums, mutation-prohibition proofs, package refs when package-supplied, and `030.VersionManifest` refs.

| Materialization item | Required value or behavior | Default when missing |
| --- | --- | --- |
| Source-dataset catalog row-set ID | `sdc-mvp-public-source-datasets-v1`. | `blocked_todo`; no selected dataset row, no production feed read/import, and no absence-sensitive effect. |
| Source-dataset selected members | Every token in `MVPSourceDatasetCatalogClosureInventory` must resolve to exactly one active `SourceDatasetCatalogRow` or exactly one deterministic source-dataset block row for the selected production scope. | `SOURCE_DATASET_CATALOG_ROW_MISSING` or `SOURCE_DATASET_DETERMINISTICALLY_BLOCKED`; no forbidden mutation. |
| Feed-category closure row-set ID | `TODO: product governance must supply active LakehouseFeedCategoryClosureRowSet ID and checksum`. | `blocked_todo`; no production feed-category activation. |
| Feed-category selected members | Every category in `LakehouseFeedCategoryClosureRequirementTable` must resolve to exactly one active `LakehouseFeedCategoryClosureRow` or exactly one deterministic category block row. | `LAKEHOUSE_FEED_CATEGORY_ROW_MISSING`, duplicate-row error, or `blocked_todo`; no absence-sensitive effect. |
| Effect closure map | Every selected feed-category row must carry a total `effect_closure_requirements` map over `absence`, `cleanup`, `retraction`, `graph_expiry`, and `watermark`. | Missing key fails validation; it must not default to positive-only. |
| Effect posture | `requires_060_closure` means the category row may ask `060` to evaluate the requested effect. | It does not authorize the effect. Authorization still requires selected `060.SourceAuthorityClosureMatrixRow` or deterministic block row plus every underlying ref. |
| Manifest inclusion | Every selected row, row-set, selector checksum, validation ref, mutation-prohibition proof, package release ref, package-set ref, and consulted `060` row must appear in `030.VersionManifest`. | `blocked_manifest` before output or visibility. |
| Package-supplied material | Package-supplied row catalogs and block rows require immutable `PackageReleaseManifest`, active `ProductionPackageSetManifest`, validation refs, package-set refs, compatibility refs when required, and manifest refs. | `blocked_package_set`; candidate output remains invisible and current active package set remains active. |

### Required source-dataset activation decisions

| Public `source_dataset` token | Required MVP row state | Required `permitted_production_uses` | Required closure behavior |
| --- | --- | --- | --- |
| `endpoint_inventory` | Active row when selected scope includes endpoint inventory. | `raw_import`, `silver_observation`, `positive_gold_candidate`. | Absence-sensitive effects require exact `060` closure rows. |
| `configuration_inventory` | Active row when selected scope includes configuration inventory. | `raw_import`, `silver_observation`, `positive_gold_candidate`. | Absence-sensitive effects require exact `060` closure rows. |
| `vulnerability_scan` | Active row when selected scope includes vulnerability scan data. | `raw_import`, `silver_observation`, `positive_gold_candidate`. | Fixed or absent vulnerability output requires exact coverage, staleness, completeness, authority, absence, validation, package, and manifest refs. |
| `control_evaluation` | Active row when selected scope includes control results. | `raw_import`, `silver_observation`, `positive_gold_candidate`. | Pass, fail, unknown, not-checked, and not-applicable output requires exact control-result mapping and source-effect closure rows. |
| `directory_inventory` | Active resolver-input row or deterministic block row for non-resolver effects. | `raw_import`, `silver_observation`, `resolver_input_only`. | Must not become direct gold, membership, nonmembership, deletion, cleanup, graph expiry, or graph edge removal authority. |
| `directory_membership` | Active row when selected scope includes membership facts. | `raw_import`, `silver_observation`, `positive_gold_candidate`. | Membership absence and nonmembership require exact `060` closure and hidden-membership or permission coverage rows. |
| `dns_record_set` | Active row when selected scope includes DNS records. | `raw_import`, `silver_observation`, `positive_gold_candidate`. | DNS absence and TTL-derived effects require exact `060` staleness and authority rows. |
| `dhcp_ipam_assignment` | Active row when selected scope includes DHCP/IPAM assignments. | `raw_import`, `silver_observation`, `positive_gold_candidate`. | Lease expiry must not imply host absence without exact `060` closure. |
| `cloud_asset_inventory` | Active row when selected scope includes cloud asset inventory. | `raw_import`, `silver_observation`, `positive_gold_candidate`. | Cloud disappearance, cleanup, graph expiry, source-history consult, and watermark require exact `060` closure. |
| `network_flow` | Active positive-only row for observed flow; deterministic block row for flow absence. | `raw_import`, `silver_observation`, `positive_gold_candidate`. | Positive observed-flow only in MVP; flow absence and theoretical reachability remain blocked. |
| `source_history` | Supporting-evidence row only unless exact future no-change closure exists. | `supporting_evidence_only`. | No no-change proof or negative output without exact retention, authority, coverage, staleness, absence, validation, package, and manifest refs. |
| `future_reachability` | Exact deterministic block row while `200` remains `inactive_deferred`. | `deterministically_blocked`, `validation_only`. | No production read target, fact, graph edge, graph property, API claim, package activation effect, or production reachability validation pass. |

### Feed-category effect-map closure

`ValidateMVPFeedCategoryClosure` must fail before feed activation when any selected category row omits an effect key, uses an unknown effect key, duplicates an effect key after canonicalization, contains `TODO:` in a required effect cell, names a `requires_060_closure` cell without exact `060` refs or deterministic block refs, or lacks mutation-prohibition proof for a blocked effect.

Acceptance criteria:

| ID | Requirement |
| --- | --- |
| `020-MVP-ACTIVATION-CATALOG-MATERIALIZATION-AC-001` | Every MVP `source_dataset` token and every referenced extra token resolves to exactly one active `SourceDatasetCatalogRow` or exactly one deterministic source-dataset block row. |
| `020-MVP-ACTIVATION-CATALOG-MATERIALIZATION-AC-002` | Every feed category in `LakehouseFeedCategoryClosureRequirementTable` resolves to exactly one active feed-category closure row or exactly one deterministic category block row. |
| `020-MVP-ACTIVATION-CATALOG-MATERIALIZATION-AC-003` | Every selected feed-category closure row has a total effect map over `absence`, `cleanup`, `retraction`, `graph_expiry`, and `watermark`. |
| `020-MVP-ACTIVATION-CATALOG-MATERIALIZATION-AC-004` | Every selected source-dataset row carries `permitted_production_uses` and validation proves that disallowed uses fail before output. |
| `020-MVP-ACTIVATION-CATALOG-MATERIALIZATION-AC-005` | Promotion and production output remain impossible while any required catalog row, block row, validation fixture, expected output/error checksum, mutation-prohibition proof, package-set ref, or manifest ref contains `TODO:`. |

## Supporting Material Blockers

The blockers below replace open product questions for this file. They are owner-local activation blockers. A downstream implementation must not resolve them by inference.

| Blocker ID | Missing artifact family | Required row-set ID or ref | Required validation rows | Required `030.VersionManifest` refs | Default until supplied |
| --- | --- | --- | --- | --- | --- |
| `020-TODO-SOURCE-DATASET-CATALOG` | Concrete public source-dataset catalog row bytes, selected row refs, row checksums, row-set checksum, deterministic source-dataset block rows, validation refs, package refs when package-supplied, and mutation-prohibition proofs. | `sdc-mvp-public-source-datasets-v1`. | `120-SOURCE-DATASET-CATALOG-*`, `120-FEED-CLOSURE-*`, `120-COVERAGE-DOMAIN-*`, `120-VERSION-MANIFEST-*`, and package-set rows when package-supplied. | Source-dataset row-set ref/checksum, selected row refs/checksums, deterministic block refs, selector checksums, validation refs, package release refs, package-set refs, mutation-prohibition refs, and owner error refs. | `blocked_todo`; referenced datasets without selected rows or block rows fail with `SOURCE_DATASET_CATALOG_ROW_MISSING` and emit no absence-sensitive mutation. |
| `020-TODO-FEED-CATEGORY-CLOSURE-ROW-SET` | Concrete feed-category closure row-set bytes, one selected row or deterministic block row per category, total effect maps, effect-map checksums, source-dataset allowlist refs, `060` closure refs, validation refs, package refs when package-supplied, and mutation-prohibition proofs. | `TODO: product governance must supply active LakehouseFeedCategoryClosureRowSet ID and checksum`. | `120-FEED-CLOSURE-*`, `120-SOURCE-CLOSURE-*`, `120-SOURCE-DATASET-CATALOG-*`, `120-VERSION-MANIFEST-*`, and package-set rows when package-supplied. | Feed-category row-set ref/checksum, selected category row refs/checksums, effect-map checksum, deterministic block refs, selected source-dataset refs, `060` closure refs or block refs, validation refs, package refs, mutation-prohibition refs, and owner error refs. | `blocked_todo`; production feed categories without active rows fail with `LAKEHOUSE_FEED_CATEGORY_ROW_MISSING` or deterministic block behavior. |
