...
## Package Activation Row Schema Closure

This section closes the package activation row-family interface. A package row family that can affect activation, rollback, quarantine, emergency behavior, package stage execution, health, audit, validation acceptance, or package-visible diagnostics must resolve through one of the closed row-family states exported by `030.ActivationControlledRowSchema`. A package release, package-set manifest, validation report, registry label, policy bundle, repository snapshot, or owner prose must not substitute for selected row refs, selected row checksums, row-set checksums, validation refs, package-set refs, generated error-registry refs, or `030.VersionManifest` inclusion.

### PackageActivationRowFamilyClosureTable

| row_family | production_effect | closure_state | required selected refs | unresolved supporting-material behavior |
| --- | --- | --- | --- | --- |
| `PackageTypePolicyRow` | package type eligibility, stage/output permissions, supply-chain evidence requirements, rollback, LKG, quarantine, emergency scope | `full_row_schema` | selected row ref/checksum, row-set ref/checksum, selector context/checksum, validation refs, package-set refs when package-supplied | Missing concrete rows block with `PACKAGE_TYPE_POLICY_MISSING`; ambiguous concrete rows block with `PACKAGE_TYPE_POLICY_AMBIGUOUS`. |
| `PackageReleaseManifest` | release eligibility and release checksum | `full_row_schema` | release manifest ref/checksum, immutable artifact refs, selected policy refs, supply-chain evidence refs, compatibility refs, validation refs | Missing concrete release manifest blocks with `PACKAGE_ACTIVATION_ARTIFACT_MISSING` or the more specific owner code. |
| `ProductionPackageSetManifest` | only production activation target | `full_row_schema` | package-set manifest ref/checksum, release refs/checksums, selected row refs/checksums, lifecycle, approval, validation, rollback, quarantine, emergency, LKG refs | Missing concrete package-set manifest blocks with `PACKAGE_SET_CHECKSUM_MISMATCH` or `PACKAGE_VERSION_MANIFEST_INCOMPLETE` by failure point. |
| `PackageRepositoryModelRow` | repository form, metadata, snapshot, freshness, anti-rollback behavior | `full_row_schema` | repository model row ref/checksum, metadata refs, snapshot refs, freshness proof refs, anti-rollback state refs | No production default repository model exists; missing rows block with `PACKAGE_REPOSITORY_FORM_UNSUPPORTED` or `PACKAGE_REPOSITORY_METADATA_EXPIRED`. |
| `PackageTrustPolicyRow` | trust roots, signer authorization, thresholds, rotation, compromise behavior | `full_row_schema` | trust policy row ref/checksum, trust root refs, signer refs, signature verification refs, validation refs | Missing concrete trust rows block activation; inactive roots and unauthorized signers are non-bypassable. |
| `PackageTransparencyEvidencePolicyRow` | transparency proof requirement and provider support | `full_row_schema` | transparency policy row ref/checksum, proof refs, subject digest refs, validation refs | Missing required transparency evidence blocks with `PACKAGE_TRANSPARENCY_EVIDENCE_MISSING`. |
| `PackageProvenancePolicyRow` | builder/material/product provenance acceptance | `full_row_schema` | provenance policy row ref/checksum, build provenance refs, attestation refs, validation refs | Missing or subject-mismatched provenance blocks before activation. |
| `PackageSBOMPolicyRow` | SBOM subject, component, dependency, license, and vulnerability gates | `full_row_schema` | SBOM policy row ref/checksum, SBOM refs, license/vulnerability gate refs, validation refs | Missing or policy-failed SBOM blocks before activation. |
| `PackageDependencyLockPolicyRow` | reproducible dependency set and live-resolution rejection | `full_row_schema` | dependency lock policy row ref/checksum, lock checksum refs, resolved dependency digest refs, validation refs | Missing lock, mismatch, or live resolution blocks before activation. |
| `PackageCompatibilityMatrixRow` | compatibility decision across all selected axes | `full_row_schema` | compatibility row refs/checksums for every required axis, validation refs, package-set refs | Missing, failed, expired, unmanifested, or package-set-mismatched rows block with `PACKAGE_COMPATIBILITY_FAILED`. |
| `PackageDeprecationWindowPolicyRow` | deprecation windows and expiry behavior | `full_row_schema` | deprecation row ref/checksum per package type and target environment, validation refs | No implementation may invent deprecation windows; missing rows keep policy selection blocked. |
| `RollbackCompatibilityPolicy` | immutable rollback target and rollback preflight gates | `full_row_schema` | rollback policy ref/checksum, target package-set checksum, replay/schema/graph/trust/quarantine refs | Mutable rollback targets and missing verified target refs block before active state changes. |
| `QuarantineScopePolicy` | quarantine target scope and dependent activation blocking | `full_row_schema` | quarantine scope policy ref/checksum, quarantine record refs, validation refs | Missing or active quarantine rows block dependent activation without mutating evidence. |
| `LastKnownGoodHealthGate` | last-known-good marking after activation | `full_row_schema` | health gate ref/checksum, post-activation health refs, lifecycle refs, validation refs | Active package set does not imply LKG; stale or failed health blocks LKG marking. |
| `EmergencyPackageOverrideRecord` | bounded emergency quarantine, retirement, abort, verified rollback, or deprecation extension | `full_row_schema` | emergency record ref/checksum, action refs, approver refs, expiry refs, audit refs | Emergency rows must not bypass signature, trust, repository metadata, freshness, transparency, or anti-rollback checks for new production activation. |
...
The field tables in this document use the exact `030.ActivationControlledRowField` column order. Unless a row-family-specific field table narrows a value, these defaults apply to every field in an output-affecting `100` row family.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `row_id` | `040.ScalarType.string` | `yes` | `none` | `no` | `no` | non-empty, owner-scoped, stable | `n/a` | `reject` | `n/a` | `ordered:1` | `yes` | `closed` | `110` | selected row ref and checksum required when row affects output | `PACKAGE_ACTIVATION_ARTIFACT_MISSING` | `PACKAGE_ACTIVATION_ARTIFACT_CHECKSUM_MISMATCH` |
| `validation_refs` | `array<030.ActivationControlledArtifactRef>` | `yes` | `none` | `no` | `no` | non-empty for production rows | `canonical_set` | `reject` | `artifact_ref` | `no` | `yes` | `closed` | `110` | every validation ref and checksum required | `PACKAGE_VALIDATION_FAILED` | `PACKAGE_VALIDATION_FAILED` |
| `activation_scope` | `030.ActivationScope` | `yes` | `none` | `no` | `no` | must cover request scope through the owner selector context | `n/a` | `reject` | `n/a` | `ordered:99` | `yes` | `closed` | `110` | selector context ref and selector checksum required | `PACKAGE_ACTIVATION_ARTIFACT_SCOPE_MISMATCH` | `PACKAGE_ACTIVATION_ARTIFACT_SCOPE_MISMATCH` |
| `lifecycle_status` | `030.LifecycleStatus` | `yes` | `none` | `no` | `no` | production selection requires `active` unless the owner row defines rollback/deprecation eligibility | `n/a` | `reject` | `n/a` | `no` | `yes` | `closed` | `110` | lifecycle transition evidence required when status affects selection | `PACKAGE_ACTIVATION_ARTIFACT_MISSING` | `PACKAGE_LIFECYCLE_ILLEGAL_TRANSITION` |
| `*_refs` | `array<030.ActivationControlledArtifactRef or 030.ActivationControlledRowRef or EvidenceRef>` | `conditional:owner row requires the evidence class` | `[]` only when the row explicitly declares not applicable | `no` | `no` | every ref must be immutable and checksum-valid | `canonical_set` | `reject` | `ref_id` | `no` | `yes` | `closed` | `110` | every selected ref and checksum required | `PACKAGE_ACTIVATION_ARTIFACT_MISSING` | `PACKAGE_ACTIVATION_ARTIFACT_CHECKSUM_MISMATCH` |
| `*_checksum` | `040.ScalarType.sha256` | `conditional:field names a checksummed artifact or row set` | `none` | `no` | `no` | lowercase SHA-256 hex | `n/a` | `reject` | `n/a` | `no` | `yes` | `closed` | `110` | checksum value required | `PACKAGE_SET_CHECKSUM_MISMATCH` | `PACKAGE_SET_CHECKSUM_MISMATCH` |

A row-family-specific field table that omits a field precision row for an output-affecting field must fail row-set validation with `PACKAGE_ROW_SCHEMA_INCOMPLETE` before activation. The defaults above are precision defaults, not concrete supporting row instances.
...
### PackageTypePolicyRow activation-controlled field precision

The table below supplements the synopsis above and is the authoritative `030.ActivationControlledRowField` table for `PackageTypePolicyRow`.

| field_path | type | required | default | null_allowed | omit_allowed | bounds | array_semantics | duplicate_policy | canonical_sort_key | id_input | checksum_input | extension_policy | redaction_owner | version_manifest_requirement | missing_error | invalid_error |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `row_id` | `040.ScalarType.string` | `yes` | `none` | `no` | `no` | non-empty stable row ID scoped to row set | `n/a` | `reject` | `n/a` | `ordered:1` | `yes` | `closed` | `110` | selected row ref/checksum required | `PACKAGE_TYPE_POLICY_MISSING` | `PACKAGE_ACTIVATION_ARTIFACT_CHECKSUM_MISMATCH` |
| `package_type` | `100.PackageType` | `yes` | `none` | `no` | `no` | one confirmed enum token | `n/a` | `reject` | `n/a` | `ordered:2` | `yes` | `closed` | `110` | selected policy row and package type coverage checksum required | `PACKAGE_TYPE_POLICY_MISSING` | `PACKAGE_TYPE_UNKNOWN` |
| `public_api_boundary` | `040.ScalarType.string or 030.ActivationControlledArtifactRef` | `yes` | `none` | `no` | `no` | non-empty owner contract ref or bounded prose contract hash | `n/a` | `reject` | `n/a` | `no` | `yes` | `closed` | `110` | public API boundary ref/checksum required when compatibility depends on it | `PACKAGE_TYPE_POLICY_MISSING` | `PACKAGE_COMPATIBILITY_FAILED` |
| `runtime_protocol` | `owner enum` | `no` | `none` | `no` | `yes` | `none` or owner-declared protocol token | `n/a` | `reject` | `n/a` | `no` | `yes` | `closed` | `110` | required when executable package compatibility depends on runtime protocol | `PACKAGE_TYPE_POLICY_MISSING` | `PACKAGE_COMPATIBILITY_FAILED` |
| `allowed_stage_classes` | `array<030 stage class token>` | `no` | `[]` | `no` | `yes` | subset of declared `030` stage classes | `canonical_set` | `reject` | `stage_class` | `no` | `yes` | `closed` | `110` | required when package can execute in a stage | `PACKAGE_TYPE_POLICY_MISSING` | `FORBIDDEN_STAGE_OUTPUT` |
| `allowed_output_record_classes` | `array<040 record class token>` | `no` | `[]` | `no` | `yes` | subset of owner-exported production record classes | `canonical_set` | `reject` | `record_class` | `no` | `yes` | `closed` | `110` | required when package can emit records | `PACKAGE_TYPE_POLICY_MISSING` | `FORBIDDEN_STAGE_OUTPUT` |
| `dependency_lock_policy` | `owner enum` | `no` | `required_when_dependencies_affect_output_or_validation` | `no` | `yes` | `not_applicable_declarative_only`, `required`, `required_when_dependencies_affect_output_or_validation` | `n/a` | `reject` | `n/a` | `no` | `yes` | `closed` | `110` | dependency lock policy row/ref required when value requires it | `PACKAGE_DEPENDENCY_LOCK_MISSING` | `PACKAGE_DEPENDENCY_LOCK_MISMATCH` |
| `validation_matrix_requirements` | `array<120 validation family token>` | `yes` | `none` | `no` | `no` | non-empty; must include positive, negative, replay, rollback, forbidden-output, package-specific families | `canonical_set` | `reject` | `validation_family` | `no` | `yes` | `closed` | `110` | every selected validation family ref required | `PACKAGE_VALIDATION_FAILED` | `PACKAGE_VALIDATION_FAILED` |
| `schema_compatibility_inputs` | `array<030.ActivationControlledArtifactRef>` | `no` | `[]` | `no` | `yes` | required when schema output can change | `canonical_set` | `reject` | `artifact_ref` | `no` | `yes` | `closed` | `110` | required when schema output can change | `PACKAGE_COMPATIBILITY_FAILED` | `PACKAGE_COMPATIBILITY_FAILED` |
| `graph_compatibility_inputs` | `array<030.ActivationControlledArtifactRef>` | `no` | `[]` | `no` | `yes` | required when graph output, graph query, graph apply, or analysis output can change | `canonical_set` | `reject` | `artifact_ref` | `no` | `yes` | `closed` | `110` | required when graph behavior can change | `PACKAGE_COMPATIBILITY_FAILED` | `PACKAGE_COMPATIBILITY_FAILED` |
| `trust_policy_requirement` | `owner enum` | `no` | `required` | `no` | `yes` | `required`, `not_applicable_declarative_only`, or deterministic block | `n/a` | `reject` | `n/a` | `no` | `yes` | `closed` | `110` | trust policy refs required unless row explicitly blocks activation | `PACKAGE_SIGNER_UNAUTHORIZED` | `PACKAGE_TRUST_ROOT_INACTIVE` |
| `attestation_requirement` | `owner enum` | `no` | `required` | `no` | `yes` | `required` or `not_applicable_declarative_only` | `n/a` | `reject` | `n/a` | `no` | `yes` | `closed` | `110` | attestation refs required when required | `PACKAGE_ATTESTATION_MISSING` | `PACKAGE_ATTESTATION_SUBJECT_MISMATCH` |
| `sbom_requirement` | `owner enum` | `no` | `required_for_code_or_dependencies` | `no` | `yes` | `required`, `required_for_code_or_dependencies`, `not_applicable_declarative_only` | `n/a` | `reject` | `n/a` | `no` | `yes` | `closed` | `110` | SBOM refs required when required | `PACKAGE_SBOM_MISSING` | `PACKAGE_SBOM_POLICY_FAILED` |
| `repository_form_allowlist` | `array<PackageRepositoryModel>` | `no` | `oci_digest`, `tuf_compatible_metadata`, `local_bundle` | `no` | `yes` | production forms exclude `git_tree_snapshot` | `canonical_set` | `reject` | `repository_form` | `no` | `yes` | `closed` | `110` | repository model row refs required | `PACKAGE_REPOSITORY_FORM_UNSUPPORTED` | `PACKAGE_REPOSITORY_FORM_UNSUPPORTED` |
| `deprecation_policy_ref` | `030.ActivationControlledRowRef` | `yes` | `none` | `no` | `no` | row family `PackageDeprecationWindowPolicyRow` | `n/a` | `reject` | `n/a` | `no` | `yes` | `closed` | `110` | deprecation row ref/checksum required | `PACKAGE_TYPE_POLICY_MISSING` | `PACKAGE_DEPRECATION_WINDOW_EXPIRED` |
| `rollback_compatibility_policy_ref` | `030.ActivationControlledRowRef` | `yes` | `none` | `no` | `no` | row family `RollbackCompatibilityPolicy` | `n/a` | `reject` | `n/a` | `no` | `yes` | `closed` | `110` | rollback policy ref/checksum required | `PACKAGE_ROLLBACK_TARGET_UNVERIFIED` | `PACKAGE_ROLLBACK_STATE_INCOMPATIBLE` |
| `last_known_good_health_gate_ref` | `030.ActivationControlledRowRef` | `yes` | `none` | `no` | `no` | row family `LastKnownGoodHealthGate` | `n/a` | `reject` | `n/a` | `no` | `yes` | `closed` | `110` | health gate ref/checksum required | `PACKAGE_LKG_HEALTH_GATE_FAILED` | `PACKAGE_LKG_HEALTH_GATE_FAILED` |
| `quarantine_scope_policy_ref` | `030.ActivationControlledRowRef` | `yes` | `none` | `no` | `no` | row family `QuarantineScopePolicy` | `n/a` | `reject` | `n/a` | `no` | `yes` | `closed` | `110` | quarantine policy ref/checksum required | `PACKAGE_QUARANTINE_BLOCKED_ACTIVATION` | `PACKAGE_QUARANTINE_BLOCKED_ACTIVATION` |
| `emergency_override_allowed_effects` | `array<owner enum>` | `yes` | `none` | `no` | `no` | subset of `quarantine`, `retire`, `abort`, `verified_rollback`, `deprecation_extension` | `canonical_set` | `reject` | `effect` | `no` | `yes` | `closed` | `110` | emergency effect set required when emergency action can affect eligibility | `EMERGENCY_PACKAGE_BYPASS_FORBIDDEN` | `EMERGENCY_PACKAGE_BYPASS_FORBIDDEN` |
| `validation_refs` | `array<030.ActivationControlledArtifactRef>` | `yes` | `none` | `no` | `no` | non-empty | `canonical_set` | `reject` | `artifact_ref` | `no` | `yes` | `closed` | `110` | every validation ref/checksum required | `PACKAGE_VALIDATION_FAILED` | `PACKAGE_VALIDATION_FAILED` |
| `activation_scope` | `030.ActivationScope` | `yes` | `none` | `no` | `no` | must cover target environment selector | `n/a` | `reject` | `n/a` | `ordered:3` | `yes` | `closed` | `110` | selector context and selector checksum required | `PACKAGE_TYPE_POLICY_MISSING` | `PACKAGE_ACTIVATION_ARTIFACT_SCOPE_MISMATCH` |
| `lifecycle_status` | `030.LifecycleStatus` | `yes` | `none` | `no` | `no` | production resolution requires `active` | `n/a` | `reject` | `n/a` | `no` | `yes` | `closed` | `110` | lifecycle evidence required when status changed | `PACKAGE_TYPE_POLICY_MISSING` | `PACKAGE_LIFECYCLE_ILLEGAL_TRANSITION` |

### PackageTypePolicyRowSet schema

`PackageTypePolicyRowSet` must contain rows for exactly one row family: `PackageTypePolicyRow`. The row-set checksum is SHA-256 over the canonical row-set envelope and canonical rows sorted by `(package_type, activation_scope.selector_checksum, row_id)`. Duplicate `(package_type, activation_scope.selector_checksum)` rows reject with `PACKAGE_TYPE_POLICY_AMBIGUOUS` unless one row is an exact deterministic block row and the other is inactive for the selected target environment.

For every confirmed `PackageType` token and every target environment in implementation scope, exactly one active `PackageTypePolicyRow` or exactly one deterministic block row must resolve. Unknown package types remain `PACKAGE_TYPE_UNKNOWN`. Known but uncovered package types remain `PACKAGE_TYPE_POLICY_MISSING`. Equally specific matches remain `PACKAGE_TYPE_POLICY_AMBIGUOUS`. Row-set validation must include the selected row refs, row checksums, row-set checksum, selector context refs, validation refs, package-set refs when package-supplied, and generated error-registry refs in `030.VersionManifest` when visible diagnostics or package output can be emitted.
...
| `external_schema_profile` | Package type policy row ref/checksum, `PackageReleaseManifest`, `ProductionPackageSetManifest`, artifact digest, selected profile row refs/checksums, compatibility refs, validation refs, lifecycle transition evidence, package-set checksum, and `030.VersionManifest` refs. | `OCSF package`, `schema package`, package name, module name, artifact filename, repository path, branch, tag, version string, validation report, dependency lock, SBOM, or provenance record alone. |
| `external_schema_artifact_ref` | Same package-set evidence plus source tag, source commit, compiler ref/checksum, validator ref/checksum, compiled artifact checksum, profile set checksum, extension set checksum, and class allowlist checksum. | `schema artifact`, `OCSF artifact`, repository path, branch, tag, version string, release note, validation report, dependency lock, SBOM, or provenance record alone. |
| `profile_resolution_manifest` | Same package-set evidence plus resolved inheritance, profile, required-field, recommended-field, constraint, object-path, enum, deprecated-field, and checksum refs. | `profile package`, `schema package`, artifact filename, repository path, branch, tag, version string, validation report, or summary manifest alone. |
| `observation_to_ocsf_mapping_row_set` | Same package-set evidence plus selected mapping row refs/checksums, row-set checksum, discriminator fixtures, class allowlist refs, source-dataset catalog refs, policy refs, validation refs, lifecycle transition evidence, and manifest refs. | `mapping package`, `OCSF package`, `policy_bundle`, package name, module name, artifact filename, repository path, branch, tag, version string, validation report, dependency lock, SBOM, or provenance record alone. |
| `external_enum_mapping_rule_set` | Same package-set evidence plus enum rule row refs/checksums, unknown/Other/deprecated/sibling fixtures, and manifest refs. | `enum package`, `mapping package`, row-set checksum alone, validation report alone, dependency lock, SBOM, or provenance record alone. |
| `ocsf_base_event_field_policy_set` | Same package-set evidence plus policy row refs/checksums for raw/unmapped/observable/enrichment/status/severity/confidence behavior, redaction refs, fixtures, and manifest refs. | `policy package`, `OCSF policy`, `policy_bundle`, package name, validation report, dependency lock, SBOM, or provenance record alone. |
| `source_extension_field_rule_set` | Same package-set evidence plus exact field rule refs/checksums, explicit empty rule-set ref when empty, namespace/bounds/redaction/secret-scan/reserved-name fixtures, and manifest refs. | `extension package`, `custom fields package`, `mapping package`, artifact filename, repository path, branch, tag, version string, validation report, dependency lock, SBOM, or provenance record alone. |
| `observation_type_external_mapping_validation_matrix` | Same package-set evidence plus row IDs, fixture checksums, expected output/error checksums, mutation-prohibition proofs, package-set refs, and manifest refs. | `validation package`, validation report, validation run, dependency lock, SBOM, provenance record, branch, tag, or version string alone. |
| `canonical_validation_output_schema` | Same package-set evidence plus canonical validation output schema refs, deterministic output checksum refs, volatile exclusion refs, validation refs, and manifest refs. | `validation output`, validation report, package name, artifact filename, dependency lock, SBOM, or provenance record alone. |
| `mapping_bundle` | Same package-set evidence plus mapping project manifest refs, compiler pipeline refs, mapping validation rule refs, source rows, generated artifact refs, canonical validation output checksum, and manifest refs. | broad `mapping`, `schema package`, package name, module name, artifact filename, repository path, branch, tag, version string, validation report, dependency lock, SBOM, or provenance record alone. |
...
| --- | --- | --- | --- | --- | --- |
| `observability_policy_bundle` | `140.ObservabilityInstrumentationProfile`, `140.TelemetrySignalPolicy`, `140.TelemetryCorrelationPolicy`, `140.MetricInstrumentCatalog`, `140.MetricInstrumentRow`, telemetry attribute/redaction/exporter/health/replay policies | none | none or validation-only | no domain production records; may provide activation artifacts only | `140.TelemetryCorrelationPolicy` refs/checksums; metric catalog row checksums with concrete cardinality caps; exemplar policy proof with `metric_exemplars_enabled = false`; OTel environment containment proof with `otel_environment_passthrough = false`; redaction and attribute policy refs; replay exclusion refs; validation refs for `120-OBSERVABILITY-*`; package-set refs when package-supplied |
| `telemetry_runtime_distribution` | `140.TelemetryExporterProfile` and runtime telemetry exporter/Collector compatibility | OTLP-compatible telemetry export path | none | `140.TelemetryRuntimeState` only when runtime state is owner-visible | exporter protocol compatibility; retry policy bounds; queue bounds; shutdown flush bounds; no default OTel endpoint reliance; no environment-driven endpoint, header, resource, sampler, processor, reader, plugin, exemplar, or config-file activation; no direct vendor export unless a future active policy permits it; package trust, SBOM, compatibility, validation refs |
| `telemetry_collector_deployment_profile` | `140.TelemetryExporterProfile` deployment refs | Collector/runtime deployment config | none | `140.TelemetryRuntimeState` only when health-visible | opaque endpoint refs only; no raw Collector route in public artifacts; redaction before Collector handoff; network policy and trust evidence; rollback compatibility with exporter profile; private route bindings kept outside public package bytes |
...
### PackageReleaseManifest field precision closure

`PackageReleaseManifest` uses the field list above plus `PackageActivationDefaultFieldRule`. The following field groups are total for production release validation.

| Field group | Field paths | Required behavior | Missing or invalid owner error | VersionManifest requirement |
| --- | --- | --- | --- | --- |
| release identity | `package_release_manifest_id`, `package_type`, `release_version`, `release_checksum`, `activation_scope`, `lifecycle_status` | Non-null; `package_type` must be one confirmed token; `release_checksum` is SHA-256 over canonical manifest bytes excluding itself. | `PACKAGE_TYPE_UNKNOWN`, `PACKAGE_SET_CHECKSUM_MISMATCH`, or `PACKAGE_ACTIVATION_ARTIFACT_SCOPE_MISMATCH` | release manifest ref/checksum, package type policy ref/checksum, selector checksum, lifecycle evidence refs. |
| immutable artifact | `package_artifact_ref`, `artifact_digest`, `artifact_size_bytes`, `media_type`, `subject_digest` | Artifact ref must be immutable; digest, size, media type, and subject digest must match repository evidence. | `PACKAGE_ACTIVATION_ARTIFACT_MISSING` or `PACKAGE_ACTIVATION_ARTIFACT_CHECKSUM_MISMATCH` | artifact ref, artifact digest, artifact size, media type, subject digest. |
| repository evidence | `repository_form`, `repository_metadata_refs`, `repository_snapshot_refs`, `repository_freshness_proof_refs`, `repository_anti_rollback_state_refs` | `repository_form` must be allowed by the selected policy; `git_tree_snapshot` is validation-only or provenance-only and fails production activation. | `PACKAGE_REPOSITORY_FORM_UNSUPPORTED`, `PACKAGE_REPOSITORY_METADATA_EXPIRED`, or `PACKAGE_REPOSITORY_ROLLBACK_DETECTED` | repository model row refs, metadata refs, snapshot refs, freshness refs, anti-rollback refs. |
| trust and transparency | `signature_verification_result_refs`, `trust_policy_refs`, `transparency_evidence_refs` | Scalar signature summaries are derived only; signer authorization, trust-root activity, threshold rules, and transparency requirements must resolve through selected rows. | `PACKAGE_SIGNER_UNAUTHORIZED`, `PACKAGE_TRUST_ROOT_INACTIVE`, `PACKAGE_SIGNATURE_THRESHOLD_FAILED`, or `PACKAGE_TRANSPARENCY_EVIDENCE_MISSING` | trust policy refs, signature verification refs, transparency evidence refs, generated error registry checksum. |
| provenance and SBOM | `attestation_set_refs`, `build_provenance_refs`, `sbom_refs` | Subject digest must match release subject; expired, mismatched, missing, or policy-failed evidence blocks before activation. | `PACKAGE_ATTESTATION_MISSING`, `PACKAGE_ATTESTATION_SUBJECT_MISMATCH`, `PACKAGE_PROVENANCE_POLICY_FAILED`, `PACKAGE_SBOM_MISSING`, `PACKAGE_SBOM_SUBJECT_MISMATCH`, or `PACKAGE_SBOM_POLICY_FAILED` | attestation refs, provenance refs, SBOM refs, policy refs, validation refs. |
| dependency and compatibility | `dependency_lock_refs`, `compatibility_matrix_refs` | Dependency locks prove reproducibility only; every output-affecting compatibility axis required by the selected policy must pass. | `PACKAGE_DEPENDENCY_LOCK_MISSING`, `PACKAGE_DEPENDENCY_LOCK_MISMATCH`, `PACKAGE_DEPENDENCY_LIVE_RESOLUTION_FORBIDDEN`, or `PACKAGE_COMPATIBILITY_FAILED` | dependency lock refs, resolved dependency digest refs, compatibility row refs/checksums. |
| developer and stage binding | `package_developer_contract_ref`, `stage_binding_refs`, `activation_artifact_refs` | Executable or artifact-supplying packages must bind to owner contracts and stage permissions before execution or row-catalog selection. | `PACKAGE_ACTIVATION_ARTIFACT_OWNER_MISMATCH`, `PACKAGE_ACTIVATION_ARTIFACT_VALIDATION_MISSING`, or `FORBIDDEN_STAGE_OUTPUT` | developer contract ref, stage binding refs, activation artifact refs, artifact owner specs, validation refs. |
| structured-input provenance | `source_materialization_refs`, `structured_input_publication_manifest_refs`, `producer_ci_validation_refs`, `maintenance_tool_invocation_refs`, `candidate_sync_record_refs` | Repository-authored packages must materialize immutable artifact bytes; producer CI, publication, maintenance tool, and sync refs are evidence only and cannot activate by themselves. | `STRUCTURED_INPUT_MATERIALIZATION_REQUIRED`, `STRUCTURED_INPUT_PUBLICATION_MANIFEST_MISSING`, `STRUCTURED_INPUT_PRODUCER_CI_STALE`, or `STRUCTURED_INPUT_SYNC_RECORD_NONAUTHORITY` | materialization refs, publication manifest refs, producer CI refs, tool invocation refs, candidate sync refs when consulted. |
| validation | `validation_refs` | Non-empty concrete validation refs with fixture checksums, expected output/error checksums, and mutation-prohibition proofs. | `PACKAGE_VALIDATION_FAILED` | every validation ref/checksum and package-set ref when package-supplied. |

All array-valued release manifest ref fields use `canonical_set`, duplicate rejection, lexical ref sorting, and checksum inclusion. Omission, null, mutable refs, bare string refs, TODO values, or unmanifested refs reject before compatibility checks, package-set activation, rollback, quarantine, emergency, health gating, or candidate production output.
...
15a. When package-supplied artifacts are observability packages, verify they cannot set `otel_environment_passthrough = true`, enable exemplars, expose raw trace/span IDs, use diagnostic refs as authority, omit exporter defaults, use unbounded labels, weaken replay exclusion, bypass redaction, or affect health outside `140.TelemetryRuntimeState` and `110.OperationalHealthStatus`.
...
### ProductionPackageSetManifest field precision closure

Every `ProductionPackageSetManifest` field in the schema table above is output-affecting. Arrays of refs use `canonical_set`, duplicate rejection, lexical ref sorting, checksum inclusion, and `PACKAGE_SET_DUPLICATE_REF` for duplicates after canonicalization.

| Field group | Field paths | Required behavior | Missing or invalid owner error | VersionManifest requirement |
| --- | --- | --- | --- | --- |
| package-set identity | `package_set_manifest_id`, `target_environment`, `activation_mode`, `package_set_checksum` | `target_environment` materializes to `030.ScopeSelector`; `activation_mode` is closed; checksum covers canonical manifest bytes excluding itself. | `PACKAGE_SET_CHECKSUM_MISMATCH` or `PACKAGE_ACTIVATION_ARTIFACT_SCOPE_MISMATCH` | package-set manifest ref/checksum, selector context/checksum, activation mode. |
| release membership | `package_release_refs`, `release_manifest_checksums`, `cohesion_groups` | Releases must be immutable, complete, checksum-valid, and cohesion-complete before activation. | `PACKAGE_COHESION_INCOMPLETE` or `PACKAGE_SET_CHECKSUM_MISMATCH` | release refs/checksums, cohesion group refs. |
| selected policy rows | `package_type_policy_row_set_ref`, `package_type_policy_refs`, `package_type_policy_coverage_matrix_checksum`, `deprecation_policy_refs`, `repository_model_refs`, `supply_chain_policy_refs` | Every release package type must select exactly one active policy row, deprecation row, repository row, and supply-chain policy row. | `PACKAGE_TYPE_POLICY_MISSING`, `PACKAGE_TYPE_POLICY_AMBIGUOUS`, or owner-specific policy error | selected row refs/checksums, row-set checksums, coverage matrix checksum. |
| supply-chain evidence | `supply_chain_evidence_refs`, `trust_refs` | Signature, trust, transparency, attestation, provenance, SBOM, dependency lock, and repository evidence must match selected policies. | most-specific supply-chain package error | every evidence ref/checksum and generated error registry checksum when visible. |
| activation artifacts | `activation_artifact_refs`, `artifact_owner_specs`, `artifact_validation_refs`, `artifact_compatibility_refs`, `artifact_scope`, `artifact_registry_snapshot_ref` | Package-supplied artifacts may instantiate owner row schemas only through immutable refs, exact owners, validation, compatibility, scope, and registry snapshots. | `PACKAGE_ACTIVATION_ARTIFACT_MISSING`, `PACKAGE_ACTIVATION_ARTIFACT_OWNER_MISMATCH`, or `PACKAGE_ACTIVATION_ARTIFACT_SCOPE_MISMATCH` | activation artifact refs, owner spec refs, validation refs, compatibility refs, artifact scope checksum, registry snapshot ref. |
| operational gates | `health_gate_refs`, `validation_refs`, `compatibility_refs`, `rollback_refs`, `quarantine_scope_policy_refs`, `quarantine_refs`, `emergency_override_refs`, `lifecycle_transition_evidence_refs`, `approval_refs` | Health, validation, compatibility, rollback, quarantine, emergency, lifecycle, and approval gates must pass without bypassing trust. | `PACKAGE_LKG_HEALTH_GATE_FAILED`, `PACKAGE_VALIDATION_FAILED`, `PACKAGE_COMPATIBILITY_FAILED`, rollback/quarantine/emergency owner errors | all gate refs/checksums, lifecycle refs, approval refs, rollback/quarantine/emergency refs. |

A package-set manifest field is descriptive only until the same ref and checksum appears in `030.VersionManifest.included_refs`. A package set that validates as a record but omits required `VersionManifest` refs fails with `PACKAGE_VERSION_MANIFEST_INCOMPLETE` before current active pointer changes or candidate output visibility.
...
### PackageActivationFailurePrecedenceTable

Activation failure precedence is deterministic. When multiple failures are present, the implementation must emit the first applicable family below and preserve the current active package set unless the selected row explicitly permits a post-activation health policy effect.

| Precedence | Failure family | Required selected refs | Failure code family | Current-active-set behavior | Candidate-output behavior |
| ---: | --- | --- | --- | --- | --- |
| 1 | checksum, manifest ID, or canonical byte mismatch | release checksum, package-set checksum, artifact digest, row checksum | `PACKAGE_SET_CHECKSUM_MISMATCH`, `PACKAGE_ACTIVATION_ARTIFACT_CHECKSUM_MISMATCH` | keep current | write none |
| 2 | activation or run lock unavailable, lost, stale, fenced, or idempotency-conflicted | `030.RunLockOperationEvidence`, `030.RunLockCommitGuard` | `PACKAGE_ACTIVATION_IDEMPOTENCY_CONFLICT` or imported `030` lock error | keep current | write none |
| 3 | package-set cohesion incomplete | cohesion group refs, release refs | `PACKAGE_COHESION_INCOMPLETE` | keep current | write none |
| 4 | package type policy missing, inactive, ambiguous, scope-mismatched, or unmanifested | policy row-set, selected row, selector checksum | `PACKAGE_TYPE_POLICY_MISSING`, `PACKAGE_TYPE_POLICY_AMBIGUOUS`, `PACKAGE_VERSION_MANIFEST_INCOMPLETE` | keep current | write none |
| 5 | deprecation row missing or expired | deprecation row ref/checksum | `PACKAGE_TYPE_POLICY_MISSING`, `PACKAGE_DEPRECATION_WINDOW_EXPIRED` | keep current | write none |
| 6 | repository form, freshness, metadata, snapshot, or anti-rollback failure | repository model, metadata, snapshot, freshness, anti-rollback refs | `PACKAGE_REPOSITORY_FORM_UNSUPPORTED`, `PACKAGE_REPOSITORY_METADATA_EXPIRED`, `PACKAGE_REPOSITORY_ROLLBACK_DETECTED` | keep current | write none |
| 7 | trust, signature, signer, trust root, threshold, or transparency failure | trust policy, signature verification, transparency refs | `PACKAGE_SIGNER_UNAUTHORIZED`, `PACKAGE_TRUST_ROOT_INACTIVE`, `PACKAGE_SIGNATURE_THRESHOLD_FAILED`, `PACKAGE_TRANSPARENCY_EVIDENCE_MISSING` | keep current | write none |
| 8 | attestation, provenance, SBOM, or dependency lock failure | attestation, provenance, SBOM, dependency refs | `PACKAGE_ATTESTATION_*`, `PACKAGE_PROVENANCE_POLICY_FAILED`, `PACKAGE_SBOM_*`, `PACKAGE_DEPENDENCY_*` | keep current | write none |
| 9 | compatibility failure | compatibility matrix row refs | `PACKAGE_COMPATIBILITY_FAILED` | keep current | write none |
| 10 | developer contract, stage binding, activation artifact, or owner mismatch | developer contract, stage binding, artifact owner refs | `PACKAGE_ACTIVATION_ARTIFACT_*`, `FORBIDDEN_STAGE_OUTPUT` | keep current | write none |
| 11 | validation fixture, expected checksum, mutation proof, or package error parity failure | validation refs, expected checksums, error registry checksum | `PACKAGE_VALIDATION_FAILED`, `PACKAGE_ERROR_REGISTRY_PARITY_FAILED` | keep current | write none |
| 12 | release or package-set field omission | release/package-set manifest refs | `PACKAGE_VALIDATION_FAILED`, `PACKAGE_VERSION_MANIFEST_INCOMPLETE` | keep current | write none |
| 13 | rollback target or rollback compatibility failure | rollback policy, immutable target, replay/schema/graph/trust refs | `PACKAGE_ROLLBACK_*` | keep current | write none |
| 14 | quarantine blocker | quarantine scope policy and quarantine records | `PACKAGE_QUARANTINE_BLOCKED_ACTIVATION` | keep current | write none |
| 15 | emergency override violation | emergency record, action, approver, expiry refs | `EMERGENCY_PACKAGE_BYPASS_FORBIDDEN` | keep current | write none |
| 16 | post-activation LKG health failure | LKG health gate and health result refs | `PACKAGE_LKG_HEALTH_GATE_FAILED` | active-state evidence preserved as policy permits; no LKG mark | no new candidate output |

A lower-precedence failure must not mask a higher-precedence failure. Failure events must include the selected ref class that caused the failure, the candidate package-set checksum, the current active package-set checksum, mutation-prohibition evidence, and generated error-registry checksum when the event is visible through `110` or `120`.
...
The following `100` row families can affect package eligibility, release manifests, package-set activation, trust, provenance, SBOM, dependency locks, compatibility, deprecation, rollback, quarantine, emergency behavior, last-known-good status, package stage binding, or package-supplied row catalogs. Each output-affecting family now resolves through `PackageActivationRowFamilyClosureTable`, `PackageActivationDefaultFieldRule`, and its row-family-specific schema or field-group precision table. Concrete supporting row catalogs and validation fixture bytes remain separate promotion blockers.

| row_family | production classification | required precision status |
| --- | --- | --- |
| `PackageTypePolicyRow` | output_affecting | `full_row_schema`; see `PackageTypePolicyRow activation-controlled field precision` and `PackageTypePolicyRowSet schema`. |
| `PackageReleaseManifest` | output_affecting | `full_row_schema`; see `PackageReleaseManifest field precision closure`. |
| `ProductionPackageSetManifest` | output_affecting | `full_row_schema`; see `ProductionPackageSetManifest field precision closure`. |
| `PackageTrustPolicyRow` | output_affecting | `full_row_schema`; field paths use the `PackageTrustPolicyRow schema` table plus `PackageActivationDefaultFieldRule`. |
| `PackageRepositoryModelRow` | output_affecting | `full_row_schema`; repository forms, evidence attachment, metadata, snapshot, freshness, anti-rollback, and unsupported behavior are closed by `PackageRepositoryModel catalog` plus `PackageActivationDefaultFieldRule`. |
| `PackageTransparencyEvidencePolicyRow` | output_affecting | `full_row_schema`; field paths use `PackageTransparencyEvidencePolicyRow schema` plus `PackageActivationDefaultFieldRule`. |
| `PackageProvenancePolicyRow` | output_affecting | `full_row_schema`; field paths use `PackageProvenancePolicyRow schema` plus `PackageActivationDefaultFieldRule`. |
| `PackageSBOMPolicyRow` | output_affecting | `full_row_schema`; field paths use `PackageSBOMPolicyRow schema` plus `PackageActivationDefaultFieldRule`. |
| `PackageDependencyLockPolicyRow` | output_affecting | `full_row_schema`; field paths use `PackageDependencyLockPolicyRow schema` plus `PackageActivationDefaultFieldRule`; live dependency resolution remains forbidden. |
| `PackageCompatibilityMatrixRow` | output_affecting | `full_row_schema`; compatibility axes in `PackageCompatibilityMatrixRow schema` are selected refs and checksums, not labels. |
| `PackageDeprecationWindowPolicyRow` | output_affecting | `full_row_schema`; missing concrete row instances remain a supporting-material blocker and no deprecation window may be invented. |
| `RollbackCompatibilityPolicy` | output_affecting | `full_row_schema`; immutable rollback target, replay, schema, graph, trust, and quarantine refs must be selected and manifest-included. |
| `QuarantineScopePolicy` | output_affecting | `full_row_schema`; quarantine target scope and dependent activation blocking must be selected and manifest-included. |
| `LastKnownGoodHealthGate` | output_affecting | `full_row_schema`; active package set is not LKG until selected health rows pass. |
| `EmergencyPackageOverrideRecord` | output_affecting | `full_row_schema`; allowed effects, approver refs, expiry, target refs, trust-bypass prohibition, and audit refs must be selected and manifest-included. |
| `PackageActivationFailureEvent` | output_affecting runtime_state_record | `full_row_schema`; event refs and checksums are required for failed activation visibility. |
...
Package activation, rollback, quarantine, emergency override, package stage execution, or package-supplied row catalog selection must fail before active pointer changes or candidate output when any selected `100` row family lacks concrete supporting row bytes, uses a bare string row ref, omits package-set refs, omits validation refs, contains `TODO:` in fixture or expected-output checksums, or omits selected row refs from `030.VersionManifest`.
...
| `100-OBSERVABILITY-PACKAGE-AC-001` | Unsafe, untrusted, incompatible, missing, ambiguous, unmanifested, package-set-mismatched, validation-missing, or weakening observability packages fail before activation and the current active package set remains active. |
| `100-OBSERVABILITY-PACKAGE-AC-002` | Observability packages cannot set `otel_environment_passthrough = true`, enable exemplars, expose raw trace/span IDs, use diagnostic refs as authority, omit exporter defaults, use unbounded labels, or weaken replay exclusion. |
...
| `100-PACKAGE-ROW-PRECISION-TODO-001` | Core row-field precision is closed by `PackageActivationRowFamilyClosureTable`, `PackageActivationDefaultFieldRule`, and the package manifest precision closures. Product governance must still supply concrete supporting row catalogs, release manifests, package-set manifests, validation fixture bytes, expected output or error bytes, mutation-prohibition proofs, package-set refs, and `030.VersionManifest` refs before authoritative promotion. Until those supporting artifacts are supplied, affected package-set activation remains blocked. |
...
| `100-TODO-PACKAGE-TYPE-POLICY-ROWS` | TODO | Concrete `PackageTypePolicyRowSet` supporting material must provide exactly one active row or one deterministic block row for every confirmed package type and target environment in implementation scope. | Product governance must register the row-set artifact, selected row refs/checksums, package-set refs when package-supplied, validation refs, generated error registry refs, and `030.VersionManifest` refs. | Package activation remains blocked with `PACKAGE_TYPE_POLICY_MISSING`, `PACKAGE_TYPE_POLICY_AMBIGUOUS`, or `PACKAGE_VERSION_MANIFEST_INCOMPLETE` until concrete rows validate. |
| `100-TODO-DEPRECATION-ROWS` | TODO | Concrete `PackageDeprecationWindowPolicyRowSet` supporting material must provide exactly one active row or one deterministic block row for every confirmed package type and target environment in implementation scope. | Product governance must register deprecation row refs/checksums, package-set refs when package-supplied, validation refs, expiry fixtures, expected errors, and `030.VersionManifest` refs. | No implementation may invent a default deprecation window. Missing rows keep package type policy selection blocked with `PACKAGE_TYPE_POLICY_MISSING`; expired active windows fail with `PACKAGE_DEPRECATION_WINDOW_EXPIRED`. |
