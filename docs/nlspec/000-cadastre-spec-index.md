...
| `graph_backend_selection_closure` | `090.GraphBackendSelectionPolicy`, `090.GraphBackendSelectionDefaultDecision`, the selected default backend profile ref and checksum, current default profile rows for `mvp-postgresql-relational-graph.v1`, optional `mvp-postgresql-age.v1` validation-only rows when selected, explicit non-default JanusGraph rows when selected, provider capability rows, `GraphBackendActivationBlockerSet` refs, package-set gates, provider support evidence refs, and `120-GRAPH-POSTGRES-*`, `120-GRAPH-AGE-*`, `120-GRAPH-BACKEND-*`, and provider-portability validation refs. |
...
| `000-GRAPH-BACKEND-DEFAULT-AC-001` | Promotion fails when graph serving is in scope and `GraphBackendSelectionPolicy`, the selected default backend profile, `mvp-postgresql-relational-graph.v1` default profile rows, provider capability rows, provider support evidence rows, package-set gates, `GraphBackendActivationBlockerSet`, `120-GRAPH-POSTGRES-*`, `120-GRAPH-BACKEND-*`, or provider-portability validation refs are missing, inactive, checksum-mismatched, failed, blocked, not run, unresolved, or `TODO`-bearing. Optional `mvp-postgresql-age.v1` rows are required only when AGE is selected or validation scope includes AGE. |
| `000-GRAPH-BACKEND-JANUSGRAPH-NONDEFAULT-AC-001` | Promotion fails when an explicit non-default JanusGraph profile is selected and its profile refs, package gates, provider capability rows, validation refs, blocker-set refs, or manifest refs are missing, inactive, checksum-mismatched, failed, blocked, not run, unresolved, or `TODO`-bearing. JanusGraph rows must not satisfy the omitted-backend default. |
