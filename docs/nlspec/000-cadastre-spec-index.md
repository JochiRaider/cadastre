...
- `ActivationCatalogClosureFamilyInventory`
...
| observability_telemetry_closure | When telemetry, observability health, API diagnostics, audit diagnostics, validation diagnostics, or telemetry runtime state visibility is in implementation scope, promotion fails if `diagnostic_correlation_ref_v1` is missing; `TelemetryCorrelationPolicy` row precision is missing when refs can be emitted; exporter default numeric values, retry, queue, batch, schedule delay, shutdown, OTel environment containment, or exemplar prohibition are missing or invalid; `otel_environment_passthrough` is not exactly `false`; `metric_exemplars_enabled` is not exactly `false`; `ObservabilityInstrumentationProfile`, `TelemetrySignalPolicy`, `TelemetryCorrelationPolicy`, `MetricInstrumentRow`, or `TelemetryExporterProfile` full row-field tables are missing; telemetry runtime-state manifest refs are missing when telemetry affects health/API/audit/validation/operator-visible output; generated telemetry error registry rows are missing; any required `120-OBSERVABILITY-*` fixture checksum, expected output checksum, expected error checksum, mutation-prohibition proof, package-set ref, manifest ref, validation ref, or row checksum is missing, stale, failed, blocked, not run, checksum-mismatched, package-set-mismatched, unmanifested, or `TODO`-bearing. |
...
| observable error selection closure | Promotion fails when API, export, health, audit, validation, telemetry-visible, package-visible, or acceptance-visible diagnostics are in implementation scope and `110.SelectObservableError` fixture rows are missing, blocked, not run, failed, stale, checksum-mismatched, generic-substituted, authorization-leaking, page-token-leaking, over the 128-error cap without `partial_result`, or `TODO`-bearing. |
...
| analysis-enrichment-lineage-registry closure | When analysis, enrichment, lineage, artifact-class substitution, registry governance, custom-property metadata, registry classification, threat-intel distribution, derived-edge output, or `130` visible diagnostics are in implementation scope, promotion fails if any required row-family precision table, volatility classification, validation row family, fixture checksum, expected output checksum, expected error checksum, mutation-prohibition proof, package-set ref when package-supplied, generated error-registry ref when visible, lifecycle ref, redaction ref, or `030.VersionManifest` ref is missing, stale, failed, blocked, not run, checksum-mismatched, package-set-mismatched, unmanifested, or `TODO`-bearing. |

### RunLockClosurePromotionRefSet

When run-lock-governed output is in implementation scope, `SpecSetVersion.validation_matrix_refs` must include every row below for the active `SpecSetVersion`. A wildcard row-family ref, validation summary, acceptance summary, package label, owner prose, research report, ADR, or registry checksum alone must not substitute for exact row refs.

| Required ref class | Required row IDs |
| --- | --- |
| `030` runtime closure rows | `val-030-runlock-lease-defaults`, `val-030-runlock-timing-bounds`, `val-030-runlock-all-or-nothing`, `val-030-runlock-conflict-active`, `val-030-runlock-store-time-unavailable`, `val-030-runlock-heartbeat-refresh`, `val-030-runlock-heartbeat-timeout`, `val-030-runlock-heartbeat-failed`, `val-030-runlock-lost-before-commit`, `val-030-runlock-stale-recovery-success`, `val-030-runlock-stale-recovery-race`, `val-030-runlock-old-holder-fenced`, `val-030-runlock-idempotent-retry`, `val-030-runlock-idempotency-conflict`, `val-030-runlock-commit-guard-missing`, `val-030-runlock-assertion-failed`, `val-030-runlock-manifest-completeness`, `val-030-runlock-watermark-blocked` |
| cross-owner mutation-prohibition rows | `val-020-runlock-maintenance-loss`, `val-060-runlock-signal-nonauthority`, `val-080-runlock-replay-missing-evidence`, `val-090-runlock-graph-apply-resume`, `val-100-runlock-package-activation-loss` |
| observable error and state rows | `val-110-runlock-error-registry`, `val-110-runlock-state-rendering` |
| telemetry rows | `val-140-runlock-telemetry-cardinality`, `val-140-runlock-telemetry-nonauthority` |

Promotion must mechanically fail unless each required row has `blocking_status = pass`, concrete fixture checksum, concrete input checksum, concrete expected output checksum or expected error checksum, mutation-prohibition proof for negative or no-op rows, package-set ref when applicable, `030.VersionManifest` refs for every output-affecting row, generated `110.ErrorCodeRegistryRow` refs when visible diagnostics can be emitted, `080` replay sufficiency refs for replay rows, and `140` telemetry policy and runtime-state refs where telemetry-visible output is evaluated.

`ValidateSpecSet` must fail with `DOMAIN_OWNER_STATUS_CONTRADICTION` if `domain.md` marks run-lock closure `resolved_owner_routed` while any required run-lock row remains blocked, not run, failed, stale, checksum-mismatched, package-set-mismatched, unmanifested, or `TODO`-bearing. The same failure applies when `000`, `030`, `120`, and `domain.md` report different run-lock closure status for the same `SpecSetVersion`.
...
| `observability_telemetry_closure` | `140.diagnostic_correlation_ref_v1`, selected `TelemetryCorrelationPolicy`, `ObservabilityInstrumentationProfile`, `TelemetrySignalPolicy`, `MetricInstrumentRow`, `TelemetryExporterProfile`, `TelemetryAttributePolicy`, `TelemetryRedactionPolicy`, `TelemetryHealthMappingPolicy`, `TelemetryReplayExclusionPolicy`, telemetry runtime-state refs, generated telemetry error registry rows, package release refs and `100.ProductionPackageSetManifest` refs when package-supplied, concrete `120-OBSERVABILITY-*` fixture and expected-output/error checksums, mutation-prohibition proofs, and `030.VersionManifest` refs. |
...
`ActivationCatalogClosureFamilyInventory` is the define-once governance inventory for activation-catalog closure. It defines promotion coverage only. It must not define runtime row schemas, owner algorithms, package activation, API rendering, or validation harness mechanics.

For each selected production scope, every implementation-scoped `closure_family` below must resolve to exactly one inventory row. The inventory row must point to either one `closed_active` selected row set or one `closed_deterministically_blocked` selected deterministic block row set. Missing, ambiguous, inactive, unvalidated, `TODO:`-bearing, checksum-mismatched, package-set-mismatched, private-leaking, unmanifested, or unregistered supporting material must fail promotion before any dependent production output is visible.

| closure_family | owner_spec | implementation_scope_predicate | required_selected_row_set_ref | required_selected_row_refs | required_block_row_refs | required_validation_refs | required_package_refs | required_manifest_refs | promotion_failure_when_missing |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `source_dataset_catalog` | `020` | Any feed profile, source-authority row, mapping row, graph handoff, analysis handoff, API filter, validation fixture, or package catalog references `source_dataset`. | `020.SourceDatasetCatalogRowSet` or exact deterministic source-dataset block row set. | Selected `020.SourceDatasetCatalogRow` refs and checksums. | Required when a dataset token is intentionally unsupported. | `120-SOURCE-DATASET-CATALOG-*`. | Package release and package-set refs when package-supplied. | Selected row, row checksum, row-set checksum, validation refs, selector refs, and package refs in `030.VersionManifest`. | `blocked_missing_ref` or owner source-dataset error. |
| `feed_category_closure` | `020` | Any production feed category can affect read/import, absence, cleanup, retraction, graph expiry, or watermark. | `020.LakehouseFeedCategoryClosureRowSet`. | Selected category row refs and effect-map checksum. | Required for intentionally unsupported category/effect pairs. | `120-FEED-CLOSURE-*`. | Required when package-supplied. | Category row, row-set, validation, package, and effect-map refs in `030.VersionManifest`. | `blocked_missing_ref` or `LAKEHOUSE_FEED_CATEGORY_ROW_MISSING`. |
| `source_authority_closure` | `060` | Any positive fact, absence, cleanup, retraction, graph expiry, watermark, control result, no-change proof, compliance output, or authorized-negative label is in scope. | `060.SourceAuthorityClosureMatrixRowSet`. | Selected closure row plus every consulted authority, completeness, coverage, staleness, progress-signal, supplier-visibility, control-result, source-history, absence, external-schema authority, and watermark row ref/checksum. | Required for unsupported source/effect tuples. | `120-SOURCE-CLOSURE-*`, `120-COVERAGE-DOMAIN-*`, `120-LAKEHOUSE-NONAUTH-*`. | Required when package-supplied. | Selected source-dataset row and every consulted `060` row ref in `030.VersionManifest`. | `blocked_missing_ref` or owner source-authority error. |
| `ocsf_mapping_closure` | `050` | Any normalized OCSF or explicit `cadastre_only` silver output is in scope. | `050.ObservationToOCSFMappingRowSet` and active external-schema profile row sets. | Selected mapping row, schema profile, artifact, profile-resolution, enum, base-event, source-extension, and canonical validation refs/checksums. | Required for intentionally unsupported observation/discriminator tuples. | `120-OCSF-MAP-*`, `120-SOURCE-EXT-*`, `120-OCSF-NONAUTH-*`, `120-OCSF-DIRECTION-*`. | Required when package-supplied. | Mapping, profile, compiled artifact, validation, package, and canonical output refs in `030.VersionManifest`. | `blocked_missing_ref` or owner mapping error. |
| `source_extension_closure` | `050` | Any non-empty `CadastreSilverObservation.source_extension_fields` can be emitted, or an empty source-extension rule set is needed to prove no extension fields. | `050.SourceExtensionFieldRuleSet`. | Selected source-extension field rule refs or explicit empty rule-set ref/checksum. | Required for blocked extension namespaces or paths. | `120-SOURCE-EXT-*`. | Required when package-supplied. | Source-extension refs and checksums in `030.VersionManifest`. | `blocked_missing_ref` or source-extension owner error. |
| `resolver_catalog_closure` | `070` | Any identity evidence candidate, identity decision, review case, split, explanation, graph correction handoff, or target-selector behavior is in scope. | Resolver artifact row sets named by `070.MVPResolverCatalogClosureInventory`. | Selected resolver profile, evidence class, scope, candidate generation, hard blocker, generation boundary, decision matrix, confidence band, routing, split, explanation, activation report, target selector, shadow-run refs/checksums. | Required for unsupported resolver scopes. | `120-IDENTITY-CLOSURE-*`. | Required when package-supplied. | Every selected resolver artifact, validation, lifecycle, package, and runtime-state ref in `030.VersionManifest`. | `blocked_missing_ref` or owner resolver error. |
| `gold_fact_predicate_catalog_closure` | `080` | Any `GoldFact` key/id computation, correction, graph handoff, API/export fact output, or validation acceptance is in scope. | `080.GoldFactPredicateContractRowSet`. | Selected predicate row refs/checksums and authority, source dataset, temporal, structured schema, validation, and package refs. | Required for unsupported fact/predicate tuples. | `120-GOLD-PREDICATE-CATALOG-*`. | Required when package-supplied. | Predicate, source-dataset, authority, temporal, schema, validation, and package refs in `030.VersionManifest`. | `blocked_missing_ref` or predicate owner error. |
| `gold_structured_value_schema_closure` | `080` | Any gold predicate can emit structured object values. | `080.MVPGoldFactStructuredValueSchemaCatalog`. | Selected structured value schema refs/checksums, enum/bounds/normalization refs, and fixture refs. | Required when schema details remain unavailable for a predicate. | `120-GOLD-PREDICATE-CATALOG-*` and structured object schema rows in `120`. | Required when package-supplied. | Schema refs, expected output checksums, validation refs, package refs in `030.VersionManifest`. | `blocked_todo` when enum, bounds, normalization, fixture, or checksum is unresolved. |
| `temporal_policy_closure` | `080` | Any fact-time resolution, late arrival, correction, replay, graph handoff, audit reconstruction, API/export time view, or validation acceptance is in scope. | `080.TemporalSemanticsPolicy`, `080.KnowledgeTimeImportPolicy`, `080.LateArrivalPolicy`, `080.GoldFactCorrectionPolicy`, `080.CorrectionSnapshotRefPolicy`. | Selected policy row refs/checksums. | Required for unsupported temporal modes or source/fact tuples. | `120-TEMPORAL-*`, `120-TEMPORAL-CORRECTION-LAKEHOUSE-*`. | Required when package-supplied. | Selected temporal, correction, late-arrival, snapshot, and validation refs in `030.VersionManifest`. | `blocked_missing_ref` or owner temporal error. |
| `replay_output_class_closure` | `080` | Any production replay, shadow replay, graph rebuild equivalence, analysis replay, API/export replay, or validation acceptance uses output-class-specific checksums. | `080.ReplayEquivalencePolicyOutputClassRow` and owner field-selection rows. | Selected replay output-class rows and owner field-selection refs/checksums. | Required for unsupported output classes. | `120-TEMPORAL-*`, `120-ANALYSIS-REPLAY-*`, `120-VERSION-MANIFEST-*`. | Required when package-supplied. | Replay policy, input sufficiency, field-selection, expected checksum, validation, and package refs in `030.VersionManifest`. | `blocked_missing_ref` or owner replay error. |
| `graph_active_profile_closure` | `090` | Any graph delta, graph query, graph rebuild, graph apply, graph evidence, graph API/export context, or graph validation is in scope. | `090.GraphActiveProfileClosure` row sets. | Selected graph edge semantics, projection profile, output eligibility, property evidence, traversal, taxonomy, validation refs/checksums. | Required for inactive edge types, including `has_theoretical_reachability`. | `120-GRAPH-PROFILE-CLOSURE-*`. | Required when package-supplied. | Graph semantics, profile, eligibility, validation, package, and source-authority handoff refs in `030.VersionManifest`. | `blocked_missing_ref` or owner graph profile error. |
| `graph_backend_selection_closure` | `090` | Graph serving, graph apply, graph query, graph rebuild, graph backend health, or backend package cohorts are in scope. | `090.GraphBackendSelectionPolicy` and selected backend profile row sets. | Selected backend profile, provider capability, adapter, driver, schema, query translation, apply, restore, migration, benchmark, telemetry redaction, generated error, validation refs/checksums. | Required for unsupported backends or validation-only AGE scope. | `120-GRAPH-BACKEND-*`, `120-GRAPH-POSTGRES-*`, `120-GRAPH-AGE-*`, `120-GRAPH-BENCHMARK-*`. | Required when package-supplied. | Backend, package, validation, telemetry, generated-error, runtime-state refs in `030.VersionManifest`. | `blocked_missing_ref` or owner backend error. |
| `package_type_policy_closure` | `100` | Any package release, package-set activation, package-supplied catalog, stage binding, rollback, quarantine, LKG, emergency, package-visible diagnostic, or package validation is in scope. | `100.PackageTypePolicyRowSet`. | Selected package type policy rows/checksums by package type and target environment. | Required for unsupported package types/environments. | `120-PACKAGE-*`. | Package release and active package-set refs are required for package-supplied rows. | Policy, release, package-set, validation, generated-error refs in `030.VersionManifest`. | `blocked_missing_ref` or package type policy error. |
| `package_deprecation_policy_closure` | `100` | Any deprecated package can remain selectable, retire, rollback, extend deprecation, or affect compatibility. | `100.PackageDeprecationWindowPolicyRowSet`. | Selected deprecation policy row refs/checksums by package type and environment. | Required for unsupported deprecation behavior. | `120-PACKAGE-*`. | Required when package-supplied. | Deprecation row, lifecycle, validation, package-set refs in `030.VersionManifest`. | `blocked_missing_ref` or deprecation owner error. |
| `package_release_manifest_closure` | `100` | Any package release participates in validation, activation, rollback, quarantine, emergency, health, or package-visible diagnostics. | `100.PackageReleaseManifest`. | Selected immutable release manifest refs/checksums and supply-chain evidence refs. | Required for unsupported candidate release states. | `120-PACKAGE-*`. | Release manifest is required. | Release manifest, evidence, validation, error-registry refs in `030.VersionManifest`. | `blocked_missing_ref` or `PACKAGE_ACTIVATION_ARTIFACT_MISSING`. |
| `production_package_set_manifest_closure` | `100` | Any production package behavior or package-supplied row catalog is in scope. | `100.ProductionPackageSetManifest`. | Active package-set ref/checksum and selected package release refs/checksums. | Required for blocked package-set activation attempts. | `120-PACKAGE-*`. | Active package-set ref is required. | Package-set, release, validation, rollback, quarantine, LKG, error refs in `030.VersionManifest`. | `blocked_package_set`. |
| `generated_error_registry_closure` | `110` | API, export, health, audit, telemetry-visible, package-visible, validation-visible, or acceptance-visible diagnostics are in scope. | `110.ErrorCodeRegistry` generated bytes. | Generated registry rows, owner fragment refs, shared rows, owner context schemas, fixture refs, registry checksum. | Required for diagnostic-only deterministic block outputs when visible. | `120-ERROR-REGISTRY-*`. | Required when registry is package-supplied. | Generated registry checksum and every visible owner error ref in `030.VersionManifest`. | `blocked_missing_ref` or generated error registry error. |
| `analysis_registry_closure` | `130` | Analysis, enrichment, lineage, registry, derived edge, threat-intel, or analysis-visible diagnostics are in scope. | Analysis and registry row sets named by `130.AnalysisRegistryClosureMaterialization`. | Selected rule, compatibility, derived-edge, enrichment, lineage, artifact-class, registry, redaction, validation refs/checksums. | Required for unsupported analysis/enrichment/registry scopes. | `120-ANALYSIS-*`, `120-ANALYSIS-REPLAY-*`, `120-DERIVED-GRAPH-EDGE-*`, `120-LINEAGE-FACET-*`, `120-ARTIFACT-CLASS-POLICY-*`, `120-REGISTRY-*`. | Required when package-supplied. | Analysis, graph query, replay, redaction, validation, package, generated-error refs in `030.VersionManifest`. | `blocked_missing_ref` or owner analysis/registry error. |
| `observability_telemetry_closure` | `140` | Telemetry, observability health, API diagnostics, audit diagnostics, validation diagnostics, or telemetry runtime state visibility is in scope. | Observability row sets named by `140.ObservabilityActivationCatalogClosure`. | Selected instrumentation, signal, correlation, metric, attribute, redaction, exporter, health mapping, replay exclusion refs/checksums. | Required when telemetry is explicitly disabled or unsupported. | `120-OBSERVABILITY-*`. | Required when package-supplied. | Telemetry rows, redaction, runtime-state, validation, package, generated-error refs in `030.VersionManifest`. | `blocked_missing_ref`, `blocked_todo`, or owner telemetry error. |
...
When any `130` analysis, enrichment, lineage, artifact-class, registry governance, custom-property, classification, threat-intel distribution, derived-edge, or visible diagnostic output is in implementation scope, `SpecSetVersion.validation_matrix_refs` must include `120-130-ACTIVATION-ROW-SCHEMA-*`, `120-ANALYSIS-OUTPUT-AUTHORITY-*`, `120-ANALYSIS-REPLAY-*`, `120-THREAT-INTEL-*`, `120-THREAT-INTEL-DISTRIBUTION-*`, `120-LINEAGE-FACET-*`, `120-ARTIFACT-CLASS-POLICY-*`, `120-REGISTRY-GOVERNANCE-*`, `120-REGISTRY-CUSTOM-PROPERTY-*`, `120-REGISTRY-CLASSIFICATION-*`, `120-DERIVED-GRAPH-EDGE-*`, `120-130-MANIFEST-*`, and `120-130-ERROR-REGISTRY-*`. Missing, blocked, stale, failed, not-run, checksum-mismatched, package-set-mismatched, unmanifested, or `TODO`-bearing rows block authoritative promotion.

When `130` row catalogs are package-supplied, `SpecSetVersion.activation_artifact_registry_refs` must include selected `130` row-set refs, selected row refs/checksums, package release refs, package-set refs, compatibility refs, lifecycle refs, validation refs, redaction refs when required, generated error-registry refs when visible, and `030.VersionManifest` refs. A `policy_bundle`, package label, package path, package filename, repository path, version string, validation report, or owner prose must not substitute for these refs.
...
| `diagnostic_correlation_ref_v1` | `stable_core_contract` | `140` | `140` | Required when caller-visible correlation refs are emitted. |
...
| Observability instrumentation and runtime telemetry | `140` | 010,030,060,070,080,090,100,110,120,domain | runtime_observability | `stable_core_contract` | 120 | blocked_validation | production activation remains blocked until correlation, exporter defaults, row precision, redaction, cardinality, replay exclusion, API/audit, package, and validation rows pass with concrete non-`TODO` refs |
...
| `000-ANALYSIS-REGISTRY-CLOSURE-AC-001` | Promotion fails when analysis, enrichment, lineage, registry, threat-intel, artifact-class substitution, registry classification, registry custom-property, derived-edge, or analysis API output is in implementation scope and `SpecSetVersion.validation_matrix_refs` lacks `120-130-ACTIVATION-ROW-SCHEMA-*`, `120-ANALYSIS-OUTPUT-AUTHORITY-*`, `120-ANALYSIS-REPLAY-*`, `120-THREAT-INTEL-*`, `120-THREAT-INTEL-DISTRIBUTION-*`, `120-LINEAGE-FACET-*`, `120-ARTIFACT-CLASS-POLICY-*`, `120-REGISTRY-GOVERNANCE-*`, `120-REGISTRY-CUSTOM-PROPERTY-*`, `120-REGISTRY-CLASSIFICATION-*`, `120-DERIVED-GRAPH-EDGE-*`, `120-130-MANIFEST-*`, or `120-130-ERROR-REGISTRY-*` row families. |
| `000-ANALYSIS-REGISTRY-TODO-BLOCK-AC-001` | Promotion fails when `130` field precision tables, row checksums, fixture checksums, expected output checksums, expected error checksums, mutation-prohibition proofs, package-set refs, generated error-registry refs, lifecycle refs, redaction refs, validation refs, manifest refs, or owner acceptance rows contain required `TODO:` values. |
| `000-ANALYSIS-REGISTRY-VOLATILITY-AC-001` | Promotion fails when `ThreatIntelDistributionMappingPolicy`, `ThreatIntelArtifactRef`, `ThreatIntelEnrichmentRecord`, `LineageFacetMappingRow`, `ArtifactClassPolicy`, `RegistryArtifactGovernance`, `RegistryCustomPropertySchema`, `RegistryClassificationPolicy`, or any selected `130` activation-controlled artifact lacks one volatility classification, exact owner artifact ref, package-set ref when package-supplied, validation refs, lifecycle refs, or `030.VersionManifest` inclusion rule. |
...
| `000-RUNLOCK-CLOSURE-AC-001` | Promotion fails when any `RunLockClosurePromotionRefSet` row is absent, has `blocking_status` other than `pass`, lacks concrete fixture checksum, input checksum, expected output checksum or expected error checksum, lacks mutation-prohibition proof when negative or no-op, lacks package-set ref when applicable, lacks `030.VersionManifest` ref, lacks generated `110` registry ref when visible, or contains `TODO:`; row presence alone is insufficient. |
| `000-RUNLOCK-DOMAIN-CONSISTENCY-AC-001` | `ValidateSpecSet` fails with `DOMAIN_OWNER_STATUS_CONTRADICTION` when `domain.md`, `000`, `030`, or `120` disagree on run-lock closure status for the same `SpecSetVersion`. |
...
| `000-ACTIVATION-CATALOG-CLOSURE-AC-002` | `ValidateSpecSet` fails when any implementation-scoped row family in `ActivationCatalogClosureFamilyInventory` lacks exactly one inventory row for the selected production scope. |
| `000-ACTIVATION-CATALOG-CLOSURE-AC-003` | `ValidateSpecSet` fails when any inventory row lacks exact `120` validation row-family refs or exact `030.VersionManifest` inclusion requirements for selected row refs, block refs, package refs, generated error refs, and runtime-state refs. |
