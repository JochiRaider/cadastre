...
| `graph_backend_selection_closure` | backend selection policy ref, default decision ref when used, selected `GraphBackendProfile` ref and checksum, `GraphBackendSelectionDefaultDecision` ref when defaulted, `GraphBackendActivationBlockerSet` ref and checksum, PostgreSQL runtime/distribution package refs, PostgreSQL adapter and driver package refs, relational schema profile refs, relational anchor schema fingerprint refs, provider capability matrix refs, provider support evidence refs when provider support affects activation, AGE extension package/version refs when AGE is selected, AGE extension preflight refs, AGE namespace and `search_path` preflight refs, role/security negative fixture refs, restore/upgrade/migration/rebuild-plan evidence refs when activation depends on them, query translation refs, apply profile refs, taxonomy mapping refs, error mapping refs, telemetry policy refs, validation refs, and package-set refs. |
