...
Benchmark rows are validation blockers until product governance supplies concrete thresholds. Validation must return `blocked`, not `pass`, while any threshold or expected output checksum is `TODO:`.

| Benchmark tier | Required workloads | Required metrics | Threshold status |
| --- | --- | --- | --- |
| `tiny-fixture` | `node_detail`, `neighbor_expansion`, `bounded_path`, `evidence_drillback`, timeout/cancellation. | p50/p95/p99 latency, candidate overflow, and expected output checksum. | Required before functional acceptance; checksum values are `TODO:` blockers until supplied. |
| `small-validation` | Tiny workloads plus concurrent reads and concurrent apply. | p50/p95/p99 latency, throughput, timeout/cancellation, WAL volume, and index build time. | Required before canary; checksum and metric thresholds are `TODO:` blockers until supplied. |
| `medium-mvp` | Small workloads plus rebuild/import and restore rehearsal. | p50/p95/p99 latency, throughput, WAL volume, index build time, bloat/vacuum, restore time, and upgrade time. | TODO: product governance must supply pass/fail thresholds. |
| `large-stress` | Medium workloads at stress scale. | p50/p95/p99 latency, throughput, candidate overflow, WAL volume, index build time, bloat/vacuum, query timeout/cancellation, restore time, and upgrade time. | TODO: product governance must supply pass/fail thresholds. |
...
| `090` | graph query/apply and positive observed-connection projection plus backend default selection | backend ID rejection, OCSF endpoint-order direction rejection, unresolved endpoint rejection, backend taxonomy unmapped rejection, query candidate-limit rejection, page-token rejection, missing provider field rejection, unsupported provider capability, unsafe storage mode, implicit schema creation, stale schema fingerprint, full-scan SQL/Gremlin plan, PostgreSQL direct DML, unsafe `search_path`, RLS bypass, AGE mutation, AGE namespace bypass, AGE internal ID leak, restore/upgrade unverifiable state, and missing package gate | empty traversal no-path, missing-flow-role no-edge, ambiguous-flow-role no-edge, generic external graph payload no-pathfinding, graph serving disabled no-backend, AGE omitted-default no-op | active profile exact edge set, edge semantics, output eligibility, apply order, PostgreSQL schema/query/apply/restore/upgrade rows, AGE preflight/security/ID leak rows when selected, explicit non-default JanusGraph rows when selected, and provider portability | rebuild equivalence, graph profile replay, provider defaulting replay, PostgreSQL/AGE backend manifest replay, and backend blocker-set replay | raw property redaction, backend ID redaction, SQL/Cypher text redaction, PostgreSQL/AGE ID redaction, package evidence redaction | graph auth, output eligibility, PostgreSQL/AGE backend health, and provider gate visibility | reachability prohibition, endpoint-order non-authority, selector-only endpoint non-authority, backend-state non-authority, provider-default non-authority, raw SQL/Cypher/Gremlin bypass non-authority | required when owner declares activation-controlled artifacts. |
...
graph-backend-selection-janusgraph-explicit-non-default
...
| `120-GRAPH-JANUSGRAPH-*` | Required only when explicit non-default JanusGraph is selected; covers storage/index declaration, Gremlin translation, schema/index apply, stale mixed index, raw-write bypass, backend ID leak, package gate, partial apply, and rebuild equivalence. |
...
| `120-GRAPH-BACKEND-SELECTION-AC-001` | Graph serving enabled with omitted backend profile materializes `mvp-postgresql-relational-graph.v1`; graph serving disabled materializes no backend; omitted profile does not select AGE or JanusGraph; explicit required mode rejects omission. |
