...
| `RES-017-postgresql-apache-age.md` | Research report on PostgreSQL and Apache AGE. | Supporting evidence for PostgreSQL/AGE backend risk analysis when adopted by owner specs. | Runtime authority, graph model authority, identity authority, source authority, package activation authority, backend selection authority, provider support proof, or production adequacy proof. |
...
| `source_effect_closure` | Source-effect tuple authorization or deterministic block routing. | `060` with `000` tuple parity and `020` feed-category effect maps | Domain vocabulary must not substitute closure summaries for selected rows. |
...
`SourceDatasetCatalogRowSet` defines public dataset token eligibility. `LakehouseFeedCategoryClosureRowSet` defines category/effect closure. `SourceAuthorityClosureMatrixRowSet` defines exact source-effect authorization or deterministic block. None may substitute for another, and a domain-vocabulary statement that a term is resolved means owner-routed vocabulary only unless the matching owner specs and `120` validation rows close the concrete runtime rows.
...
| Source dataset catalog row | `SourceDatasetCatalogRow` is the public vendor-neutral token authority for one allowed `source_dataset` value and its feed category, source category, scope schema ref, allowed read targets, supported fact/predicate refs, coverage-domain tokens, default missing-row result, private-binding policy, validation refs, activation scope, and lifecycle status. | Concrete vendor or product source, private source route, lakehouse table name, feed category, coverage-domain token, OCSF class, package type, UI label, or implementation module name. | `020`, `120` | `020.SourceDatasetCatalogRow`; `020.SourceDatasetCatalogRowSet`; `120.SourceDatasetCatalogValidationMatrix` | `blocked_owner_catalogs` until `020`, `000`, `030`, `060`, `100`, and `120` source-dataset closure refs pass. |
...
| External schema term versus Cadastre term | OCSF and CIM terms are external schema/projection vocabulary mapped through profiles. Production OCSF mapping remains inactive until `050`, `100`, `120`, and `030` closure refs pass for the selected scope. | OCSF/CIM fields, vocabulary resolution, package labels, research reports, or ADRs as Cadastre identity, authority, completeness, fact, graph authority, or production mapping activation evidence. | `050`, `040`, `060`, `100`, `120`, `030` | `050` External Schema Profile and CIM Projection Contract; `000` promotion gates | Vocabulary resolved; production activation closure remains owner-gated. |
...
| `structured_input_maintenance_tool` | A deterministic local or CI tool surface that may produce diagnostics, validation outputs, generated artifacts, materialization inputs, and publication manifests. Tool output is evidence only until owner validation, materialization, package, and manifest gates pass. | SDK or CLI success, local workstation defaults, generated logs, or tool exit code as source authority, validation acceptance, package activation, or production approval. | `030`, `040`, `100`, `110`, `120`, `140` | `030.StructuredInputMaintenanceToolContract`; `030.StructuredInputMaintenanceToolInvocation`; `040.EvidenceArtifactClassRegistry`; `100.PackageReleaseManifest`; `120-STRUCTURED-INPUT-TOOL-*` | `blocked_validation` until deterministic tool fixtures and redaction fixtures pass. |
| `structured_input_repository_template` | The required repository layout and producer CI contract boundary for a structured-input repository. Template conformance is validation evidence only. | Template match, directory shape, generated-output root, or repository scaffold as owner row refs, package release refs, package-set activation, or production approval. | `030`, `100`, `110`, `120` | `030.StructuredInputRepositoryTemplateContract`; `120-STRUCTURED-INPUT-TEMPLATE-*` | `blocked_validation` until template conformance and non-authority fixtures pass. |
| `structured_input_producer_ci` | Producer-owned CI execution that validates an exact repository snapshot, selected paths, file manifest checksum, toolchain refs, and validation matrix refs. | CI success for branch tip, pull request, mutable ref, repository URL, or unbound toolchain as validation acceptance, package activation, or production approval. | `030`, `100`, `110`, `120` | `030.StructuredInputRepositoryCIContract`; `030.ValidateStructuredInputProducerCI`; `120-STRUCTURED-INPUT-CI-*` | `blocked_validation` until stale-CI and exact-snapshot fixtures pass. |
| `structured_input_publication_manifest` | Producer-published metadata describing immutable artifacts, digests, sizes, media types, schema versions, compatibility claims, validation refs, redaction refs, provenance, and lifecycle status. It is candidate package evidence only. | Publication manifest existence, producer claim, remote URL, artifact name, or compatibility claim as package activation or production approval. | `100`, `030`, `040`, `110`, `120` | `100.StructuredInputPublicationManifest`; `100.ImportStructuredInputPublicationManifest`; `040.EvidenceArtifactClassRegistry`; `120-STRUCTURED-INPUT-PUBLICATION-*` | `blocked_validation` until manifest-import and digest-mismatch fixtures pass. |
| `structured_input_candidate_sync` | Main-project candidate discovery and audit record produced by manifest-driven sync. It may stage a candidate package release or package-set candidate only through owner gates. | Sync record, sync status, poll result, webhook event, or discovered manifest as active production state, rollback eligibility, source authority, or package activation. | `030`, `100`, `110`, `120`, `140` | `030.StructuredInputCandidateSyncPolicy`; `030.StructuredInputCandidateSyncRecord`; `120-STRUCTURED-INPUT-SYNC-*` | `blocked_validation` until sync non-authority fixtures pass. |
| `structured_input_repository_group` | Optional group of external maintenance repositories that must be synchronized, validated, materialized, or activated coherently when cross-repository artifacts depend on one another. | Partial repository activation, independent branch status, or single-repository CI pass as coherent multi-repository activation. | `030`, `100`, `120` | `030.StructuredInputRepositoryGroup`; `030.ValidateStructuredInputMaintenanceWorkflow`; `120-STRUCTURED-INPUT-MULTIREPO-*` | `blocked_validation` until coherent-group and mismatch fixtures pass. |

| Graph backend selection default | Any named graph backend selection default may exist only as an owner-routed `090.GraphBackendSelectionPolicy` result with required `090`, `100`, and `120` gates. The current owner-routed omitted-backend candidate is `mvp-postgresql-relational-graph.v1`. | Any backend product, including PostgreSQL or AGE, as Cadastre graph semantics, fact authority, identity authority, source completeness authority, activation authority, or future-provider exclusion. | `090`, `100`, `120` | `090.GraphBackendSelectionPolicy`; `090.GraphBackendProfile`; `100.ProductionPackageSetManifest`; `120` graph backend validation rows | Resolved as owner-routed selection; activation remains validation-gated. |
| `postgresql_relational_graph_profile` | A graph backend profile that serves Cadastre graph read-model objects from PostgreSQL relational structures. | PostgreSQL table state as fact truth, identity truth, completeness truth, graph truth, or activation authority. | `090`, `100`, `120` | `090.GraphBackendProfile`; `090.GraphReadModelSchemaProfile`; `100` package gates; `120-GRAPH-POSTGRES-*` | Vocabulary only; runtime remains owner-gated. |
| `relational_graph_anchor` | Cadastre-owned relational rows that preserve graph node IDs, edge IDs, endpoints, evidence refs, apply state, and schema fingerprint evidence. | Backend-generated ID, tuple ID, sequence, OID, AGE ID, SQL cursor, or physical row locator as Cadastre identity. | `090` | `090.GraphReadModelSchemaProfile`; `090.BackendSchemaFingerprint` | Vocabulary only; runtime remains owner-gated. |
| `apache_age_graph_extension_profile` | Optional PostgreSQL extension profile for graph candidate materialization or validated graph handling. | AGE as omitted production default, graph truth, identity authority, direct mutation authority, or backend-internal ID source. | `090`, `100`, `120` | `090` AGE profile rows; `100` package gates; `120-GRAPH-AGE-*` | Vocabulary only; runtime remains owner-gated. |
| `age_graph_namespace` | AGE-created PostgreSQL namespace used only through declared backend profile and security gates. | Direct DML/DDL authority, graph mutation bypass, source truth, or user-visible namespace identity. | `090`, `110`, `120` | `090` AGE security gates; `110` redaction and errors; `120-GRAPH-AGE-*` | Vocabulary only; runtime remains owner-gated. |
| `provider_native_query_text` | SQL, Cypher, SQL/Cypher, AQL, or backend-native query material. | User-facing output, telemetry payload, audit detail, or production behavior without translation profile, redaction, authorization, and mutation-proof validation. | `090`, `110`, `130`, `140` | `090.GraphQueryTranslationProfile`; `110.RedactionPolicy`; `130.ValidateAnalysisQueryImport`; `140.TelemetryRedactionPolicy` | Vocabulary only; runtime remains owner-gated. |
| `graph_backend_provider_support_evidence` | Vocabulary label for provider, region, engine version, service tier, support status, backup/restore support, upgrade path, evidence date, expiry, package-set ref, validation refs, and redaction behavior routed to owner specs. | Runtime provider support proof, package activation proof, validation pass, API health success, or manifest completeness by vocabulary alone. | `090`, `100`, `110`, `120`, `140`, `030` | Runtime behavior in `090`; package evidence in `100`; health/API in `110`; validation in `120`; telemetry in `140`; manifest behavior in `030`. | Vocabulary only; no field shape or algorithm exported. |
| `graph_benchmark_threshold_profile` | Vocabulary label for benchmark thresholds and metric checksum refs used when graph performance gates are in promotion scope. | Candidate-limit authority, query algorithm, package activation, validation pass, or production promotion by vocabulary alone. | `090`, `100`, `120`, `030` | Threshold row behavior in `090`; package coupling in `100`; fixture and aggregate acceptance in `120`; manifest refs in `030`. | Vocabulary only; candidate caps remain in `090.GraphQueryCandidateLimitTable`. |
| `graph_restore_rehearsal_evidence` | Vocabulary label for clean-environment graph backend restore proof. | Runtime restore success, promotion, or health success by vocabulary alone. | `090`, `100`, `110`, `120`, `030` | Restore contract in `090`; package cohort compatibility in `100`; health/error rendering in `110`; validation in `120`; manifest refs in `030`. | Vocabulary only. |
| `graph_upgrade_rehearsal_evidence` | Vocabulary label for PostgreSQL major-version, AGE extension, schema migration, rollback, or rebuild migration proof. | Runtime upgrade authority, package activation, or promotion by vocabulary alone. | `090`, `100`, `110`, `120`, `030` | Upgrade contract in `090`; package compatibility in `100`; health/error rendering in `110`; validation in `120`; manifest refs in `030`. | Vocabulary only. |
| `postgresql_runtime_distribution_ref` | Vocabulary label for immutable PostgreSQL runtime distribution or provider runtime ref used by graph backend package cohorts. | Graph backend selection, provider support proof, package activation, or runtime version pinning by vocabulary alone. | `090`, `100`, `120`, `030` | Runtime selection behavior in `090`; package release and package set in `100`; validation in `120`; manifest refs in `030`. | Vocabulary only. |
| `backend_schema_fingerprint` | Vocabulary label for deterministic backend schema inventory and checksum refs. | Cadastre ID, graph truth, source authority, or schema validity by vocabulary alone. | `090`, `110`, `120`, `140`, `030` | Schema fingerprint behavior in `090`; health/error rendering in `110`; validation in `120`; telemetry redaction in `140`; manifest refs in `030`. | Vocabulary only. |
...
| Maintenance SDK/CLI output and owner validation | SDK/CLI output may be deterministic evidence, diagnostics, generated artifact input, or materialization input only after exact snapshot and tool contract validation. | Treating CLI success, local logs, generated files, or exit status as `CanonicalValidationOutput`, owner row refs, package release refs, package-set activation, or `VersionManifest` completeness. | `030`, `050`, `100`, `120` |
| Repository template conformance and owner activation | Template conformance proves only that repository layout matches an active template contract. | Treating template conformance as selected mapping rows, resolver rows, source-authority rows, graph profiles, registry activation, package release, or package-set activation. | `030`, `050`, `060`, `070`, `090`, `100`, `120`, `130` |
| Producer CI success and exact-snapshot validation | Producer CI success is valid only when bound to exact commit, tree, selected paths, file manifest checksum, toolchain refs, validation matrix refs, and produced output checksums. | Treating branch-tip CI, PR CI, stale CI, hook success, or CI status as package activation, production approval, validation acceptance, or source authority. | `030`, `100`, `110`, `120` |
| Publication manifest and package release manifest | A producer publication manifest is candidate package evidence verified by `100`. `PackageReleaseManifest` remains the immutable release interface. | Treating a producer manifest, compatibility claim, artifact URL, or producer release label as `100.PackageReleaseManifest`, `100.ProductionPackageSetManifest`, or production activation. | `100`, `030`, `040`, `120` |
| Candidate sync record and active package set | A sync record discovers or stages candidates and records audit evidence. Active production behavior changes only through immutable package-set activation. | Treating sync success, webhook delivery, poll state, or candidate discovery as active package-set mutation, rollback eligibility, source authority, graph state, or system-of-record state. | `030`, `100`, `110`, `120`, `140` |
| Multi-repository maintenance group and individual repository status | Cross-repository artifacts require coherent group validation when declared by `StructuredInputRepositoryGroup`. | Treating one repository's successful validation, CI, sync, or materialization as sufficient for group activation when dependencies cross repositories. | `030`, `100`, `120` |
...
`RawFeedManifest` and `ReplayRetentionDecision` are runtime-state records, not activation-controlled row catalogs. Their existence may support replay, validation, or audit only when the owning spec requires their refs and checksums in `030.VersionManifest`.
...
| Lakehouse table state | Table-format-native snapshot, commit, dataset, retention, maintenance, cross-table, and catalog-promotion evidence used for replayable Cadastre table reads and writes. | Lakehouse product defaults, mutable branch names, table paths, graph backend state, or maintenance success as source authority. | `LakehouseTableProfile`, `LakehouseSnapshotRef`, `DatasetVersionRef`, `LakehouseCommitRef`, `ReplayRetentionPolicy`, `TableMaintenancePolicy`, `ReplayRetentionDecision`, `CrossTableCommitProfile`, `CatalogBranchPromotionPolicy` | `020`, `030`, `060`, `080`, `090`, `100`, `120` | Owner-routed; validation remains blocked until lakehouse table-state closure rows pass. |
...
| Raw feed manifest | Per-batch or per-snapshot feed identity, counts, byte totals, hashes, time bounds, schema refs, supplier lineage, and completeness evidence refs. | Source completeness decision by itself. | `RawFeedManifest` | `020` | Resolved as a vocabulary term; runtime-state precision is blocked until `020.RawFeedManifestRuntimeStateSchema` and `120` validation rows pass. |
...
| External schema profile | Resolved vocabulary for a governed external schema baseline used for validation and mapping. It is not active production mapping evidence until the owner closure refs, package-set refs when supplied, validation refs, and `VersionManifest` refs pass. | Cadastre canonical fact, identity, source authority, graph, persistence model, package activation authority, or validation evidence by term existence. | `ExternalSchemaProfile`, `ExternalSchemaArtifactRef` | `050`, activation closure through `000`, `030`, `100`, `120` | Vocabulary resolved; activation closure remains owner-gated. |
| OCSF | Resolved vocabulary for the external schema profile target for supported `normalized_fields`. It is not active production mapping evidence by itself. | Cadastre truth layer, identity resolver, source authority, graph authority, package activation evidence, or validation fixture substitute. | OCSF profile identifiers owned by `050`. | `050`, activation closure through `000`, `030`, `100`, `120` | Vocabulary resolved; activation closure remains owner-gated. |
...
| `threat_intel_distribution_mapping_row` | Row-level mapping from one external threat-intel distribution label to visibility, export, and redaction behavior. | Fact authority, identity authority, source authority, source completeness, graph authority, package activation, watermark, production approval, or string-similarity label matching. | `130` | `130` | `120` | Vocabulary resolved; runtime closure validation-gated by `120`. |
...
| `lineage_facet_mapping_row` | Row-level lineage facet mapping from lineage system, namespace, facet name, schema checksum, and raw-facet policy to non-authoritative mapped metadata or rejection. | Evidence authority, source completeness, table snapshot proof, graph rebuild proof, fact authority, graph authority, package activation, watermark, or source authority by itself. | `130` | `130` | `120` | Vocabulary resolved; runtime closure validation-gated by `120`. |
...
| `artifact_class_policy_row` | Row-level artifact substitution policy for one artifact class, permitted substitutes, forbidden substitutes, authority class, evidence-ref policy, and substitution effect scope. | New `EvidenceRef.artifact_id.kind`, package activation authority, fact authority, graph authority, source authority, source completeness, production approval, or validation acceptance by itself. | `130` | `130`, with `040` for evidence class/kind validation | `120` | Vocabulary resolved; runtime closure validation-gated by `120`. |
| `artifact_substitution_request` | Owner-routed request to evaluate whether one artifact class may substitute for another for a declared effect scope. | Evidence authority, source truth, package activation, graph truth, production approval, validation acceptance, or implementation-local substitution. | `130` | `130`, with `040` for evidence refs | `120` | Vocabulary resolved; runtime closure validation-gated by `120`. |
...
| `registry_custom_property_attachment_target` | Closed target token for where a bounded registry custom property may attach. | Core record class, source-extension field path, graph property, fact predicate, package type, or API field by inference. | `130` | `130` | `120` | Vocabulary resolved; runtime closure validation-gated by `120`. |
| `registry_classification_policy` | Non-authoritative classification and glossary label routing. | Fact authority, identity authority, source authority, completeness, graph mutation, package activation authority, watermark authority, or production approval by itself. | `130` | `130` | `120` | Vocabulary resolved; runtime closure validation-gated by `120`. |
| `registry_label_conflict_behavior` | Closed behavior token for classification label conflicts. | Visibility widening, source authority, fact authority, production approval, graph mutation, or implementation-local precedence. | `130` | `130` | `120` | Vocabulary resolved; runtime closure validation-gated by `120`. |
| `analysis_registry_error_context` | Nested owner context for `130` generated error-registry rows. | Top-level caller fields, raw threat-intel values, raw facet bytes, registry payload bytes, backend query text, private bindings, or generic error substitution. | `130`, rendered by `110` | `130` owner context and `110.GenerateErrorCodeRegistry` | `120` | Vocabulary resolved; runtime closure validation-gated by `120`. |
...
| Raw feed manifest | Batch/snapshot manifest for lakehouse feed input. | Manifest checksum and declared feed identity. | Feed read/import lifecycle. | Referenced by raw import, completeness evidence, version manifest. | `020` | Resolved as vocabulary; runtime-state precision remains validation-blocked until `020` and `120` rows pass. |
| Lakehouse table state | Authoritative table-state evidence for reads, writes, replay, maintenance, correction, rebuild, and promotion. | Table profile refs, table-format-native snapshot/commit/dataset refs, and retention/maintenance decisions. | Table profile activation, retention decisions, maintenance no-op/refusal/commit, catalog promotion. | Supports replay, correction, graph rebuild, maintenance refusal, and catalog visibility. | `020`, `030`, `080`, `090`, `100` | Owner-routed; validation remains blocked until `120-LAKEHOUSE-TABLESTATE-*` rows pass. |
...
| --- | --- | --- | --- | --- | --- | --- |
| `DOM-TODO-ACTIVATION-CATALOG-CLOSURE` | `000.MVPActivationCatalogClosureGate`, `030.ActivationCatalogClosureManifestRule`, `120.ActivationCatalogClosureValidationMatrix`, and owner-local row catalogs in `020`, `050`, `060`, `070`, `090`, and `100` | root-domain routing only for activation-catalog closure; no runtime schema, default, algorithm, validation harness behavior, or side effect | `blocked_owner_catalogs` | owner specs must materialize active row-set refs or exact deterministic block refs for source-dataset catalog closure, feed category closure, OCSF mapping closure, source-authority closure, gold predicate catalog closure, resolver closure, graph profile/backend closure, and package activation closure. For `100`, core row-family interfaces are closed by `100.PackageActivationRowFamilyClosureTable`; concrete package policy/deprecation/trust/compatibility/release/package-set catalogs and validation fixtures remain supporting-material blockers. | not runtime | `120-DOMAIN-SECTION25-*`; `120-ACTIVATION-CATALOG-CLOSURE-*`; `120-PACKAGE-*` |
| `DOM-TODO-SOURCE-DATASET-CATALOG` | `020.SourceDatasetCatalogRow`, `020.SourceDatasetCatalogRowSet`, `020.ResolveSourceDatasetCatalogRow`, and `120.SourceDatasetCatalogValidationMatrix` | vocabulary routing only for public `source_dataset` token identity and activation-controlled catalog ownership; no runtime schema, default, algorithm, validation harness behavior, or side effect is defined by `domain.md` | `blocked_owner_catalogs` | active `SourceDatasetCatalogRowSet` refs, selected row refs/checksums, deterministic source-dataset block refs, governance-supplied artifact paths, package-set refs when package-supplied, validation refs, and `030.VersionManifest` refs must pass; this row is blocked until `020-TODO-SOURCE-DATASET-CATALOG`, `000` source-dataset closure-family refs, and `120-SOURCE-DATASET-CATALOG-*` rows all close for the same scope | validation fails | `120-DOMAIN-SOURCE-DATASET-CATALOG-*`; `120-SOURCE-DATASET-CATALOG-*` |
| `DOM-TODO-GOLD-PREDICATE-CATALOG` | `080.GoldFactPredicateContractRow`, `080.MVPGoldFactPredicateContractRowSetClosure`, and `120.GoldFactPredicateCatalogValidationMatrix` | vocabulary routing only for MVP gold predicate catalog ownership; no runtime schema, default, algorithm, validation harness behavior, or side effect is defined by `domain.md` | `blocked_owner_catalogs` | active predicate row-set refs, selected row refs/checksums, deterministic predicate block row refs/checksums, structured schema refs/checksums, package-set refs when package-supplied, validation refs, generated error registry refs, and `030.VersionManifest` refs must pass before gold derivation can promote | validation fails | `120-DOMAIN-SECTION25-*`; `120-GOLD-PREDICATE-CATALOG-*` |
| `DOM-TODO-ACTIVATION-ROW-SCHEMA-PRECISION` | `030.ActivationControlledRowSchema`, `030.ActivationControlledRowRef`, `100.PackageActivationRowFamilyClosureTable`, and owner-local row field tables | activation-controlled row-schema precision routing only; no runtime schema, default, algorithm, validation harness behavior, or side effect | `blocked_validation` | owner specs must keep production-affecting row families in the closed state set and provide complete field precision, row-set validation, owner error mapping, package-set refs when package-supplied, validation refs, and manifest refs. Package row-family interfaces are closed in `100`; concrete supporting rows and `120` fixtures remain blocked until supplied. | not runtime | `120-DOMAIN-SECTION25-*`; `120-ACTIVATION-ROW-SCHEMA-*`; `120-PACKAGE-*` |
| `DOM-TODO-130-ANALYSIS-REGISTRY-CLOSURE` | `130.AnalysisRegistryActivationControlledRowFieldRule`, `130.ArtifactClassPolicy`, `130.ThreatIntelDistributionMappingPolicy`, `130.LineageFacetMappingRow`, `130.RegistryCustomPropertySchema`, `130.RegistryClassificationPolicy`, `110.GenerateErrorCodeRegistry`, and `120.AnalysisEnrichmentLineageRegistryClosureValidationMatrix` | root-domain routing only for `130` row-family and generated-error closure; no runtime schema, default, algorithm, validation harness behavior, or side effect is defined by `domain.md` | `blocked_validation` | `120` must supply concrete fixture checksums, expected output checksums, expected error checksums, mutation-prohibition proofs, package-set refs when package-supplied, generated error-registry refs, lifecycle refs, redaction refs, and manifest refs before domain status may upgrade. | validation fails | `120-DOMAIN-SECTION25-*`; `120-130-ACTIVATION-ROW-SCHEMA-*`; `120-THREAT-INTEL-*`; `120-LINEAGE-FACET-*`; `120-ARTIFACT-CLASS-POLICY-*`; `120-REGISTRY-*`; `120-130-ERROR-REGISTRY-*` |
| `DOM-TODO-020-LAKEHOUSE-ROW-PRECISION` | `020.RawSupplierProfile`, `020.LakehouseReadPolicy`, `020.LakehouseTableProfile`, `020.ReplayRetentionPolicy`, `020.TableMaintenancePolicy`, `020.CrossTableCommitProfile`, `020.CatalogBranchPromotionPolicy`, `020.RawFeedManifest`, and `030.ActivationControlledRowSchema` | root-domain routing only; no runtime schema, default, algorithm, validation harness behavior, or side effect | `blocked_validation` | owner specs must provide full row-field precision or runtime-state precision plus validation rows | validation fails | `120-DOMAIN-SECTION25-*`; `120-LAKEHOUSE-TABLESTATE-*`; `120-ACTIVATION-ROW-SCHEMA-*` |
| `DOM-TODO-STRUCTURED-INPUT-REPOSITORY` | `030.StructuredInputRepositoryProfile`, `030.StructuredInputRepositorySnapshot`, `030.StructuredInputChangeProposal`, `030.StructuredInputMaintenanceToolContract`, `030.StructuredInputRepositoryTemplateContract`, `030.StructuredInputRepositoryCIContract`, `030.StructuredInputCandidateSyncRecord`, `030.StructuredInputRepositoryGroup`, `100.StructuredInputMaterializationResult`, `100.StructuredInputPublicationManifest`, `110.StructuredInputRepositoryAccessPolicy`, `110.StructuredInputRepositoryRedactionPolicy`, and `120.StructuredInputRepositoryValidationMatrix` | structured-input workflow validation, tool/template/CI evidence, publication handoff, sync, group coordination, and materialization handoff | `blocked_validation` | repository profile refs, snapshot refs, template refs, tool contract and invocation refs, producer CI refs, publication manifest refs, sync refs, repository group refs, materialization refs, package release refs, package-set refs, redaction fixtures, audit fixtures, telemetry fixtures, and manifest refs must pass with non-`TODO` checksums before production-affecting structured-input repository workflow is authoritative | validation fails | `120-DOMAIN-STRUCTURED-INPUT-*`; `120-STRUCTURED-INPUT-*` |
...
| `DOM-RESOLVED-006` | `080.CorrectionSnapshotRefPolicy` | stable correction snapshot reference policy routed to owner; validation remains blocked when correction event-sequence fixtures or expected checksums are TODO-bearing | `blocked_validation` | `120` must supply concrete correction, snapshot-ref, assertion-transition, mutation-prohibition, and expected checksum rows before authoritative promotion | validation fails | `120-DOMAIN-SECTION25-*`; `120-TEMPORAL-CORRECTION-*` |
| `DOM-RESOLVED-007` | `080.ReplayEquivalencePolicy` | stable replay equivalence policy routed to owner; validation remains blocked when output-class field-selection refs or expected checksums are TODO-bearing | `blocked_validation` | `080.ReplayEquivalencePolicyOutputClassRow` and `120-REPLAY-OUTPUT-CLASS-*` must pass for all active output classes, including telemetry health diagnostic and run-lock operation | validation fails | `120-DOMAIN-SECTION25-*`; `120-REPLAY-OUTPUT-CLASS-*` |
...
| `DOM-TODO-020` | `030.RunLockSet`, `030.RunLockLeasePolicy`, `030.RunLockOperationEvidence`, `030.RunLockRecoveryEvidence`, and `030.RunLockCommitGuard` | run-lock closure validation handoff family only; no runtime defaults, algorithms, error mappings, fixture shapes, or validation harness behavior | `blocked_validation` | owner behavior remains specified in `030`; validation requires the exact `120` row family `val-030-runlock-*`, `val-020-runlock-maintenance-loss`, `val-060-runlock-signal-nonauthority`, `val-080-runlock-replay-missing-evidence`, `val-090-runlock-graph-apply-resume`, `val-100-runlock-package-activation-loss`, `val-110-runlock-error-registry`, `val-110-runlock-state-rendering`, `val-140-runlock-telemetry-cardinality`, and `val-140-runlock-telemetry-nonauthority` | validation fails | `120-DOMAIN-SECTION25-*`; `120.RunLockClosureValidationRows`; `000.RunLockClosurePromotionRefSet`; `030.RunLockAcceptanceDependencyMap` |
...
| `DOM-RESOLVED-012` | `100.PackageType` | closed package type enum identity only | `resolved_owner_routed` | package type enum identity is closed; package policy, deprecation, compatibility, validation, package-set, trust, repository, SBOM, provenance, dependency lock, rollback, quarantine, LKG, emergency, error parity, and manifest blockers remain owner-local to `100` and route through `DOM-TODO-ACTIVATION-CATALOG-CLOSURE` | owner governs | `120-DOMAIN-SECTION25-*`; `120-PACKAGE-TYPE-*`; `120-PACKAGE-*` |
| `DOM-TODO-013` | `090.GraphBackendSelectionPolicy` | graph backend default selection plus production activation gating; omitted backend selection is owner-routed to `mvp-postgresql-relational-graph.v1` | `blocked_owner_catalogs` | routed through `DOM-TODO-ACTIVATION-CATALOG-CLOSURE`; production remains blocked until `090` backend profile rows, provider support evidence, schema fingerprints, restore/upgrade/benchmark rows, `100` runtime/adapter/driver/deployment package-set gates, `110` error/health rows, `120` validation rows, `140` telemetry redaction rows, and `030` manifest refs are complete | validation fails | `120-DOMAIN-SECTION25-*`; `120-GRAPH-BACKEND-SELECTION-*`; `120-GRAPH-POSTGRES-*`; `120-GRAPH-AGE-*`; `120-GRAPH-BACKEND-PACKAGE-GATE-*`; `120-GRAPH-BENCHMARK-*`; `120-VERSION-MANIFEST-*`; `120-OBSERVABILITY-REDACTION-*` |
...
Stable source-authority behavior is owned by `060.SourceAuthorityClosureMatrix`. Production promotion remains blocked until every active absence-sensitive feed category has exact closure rows or deterministic block rows validated by `120-SOURCE-CLOSURE-*`, and until `060-TODO-SOURCE-CLOSURE-ROW-SETS` and concrete equivalent source-effect blockers are resolved for the same selected scope.

`SourceDatasetCatalogRowSet` is owned by `020`. A `source_dataset` token must be resolved through exactly one active public catalog row or one exact deterministic source-dataset block row before feed activation, mapping activation, source-authority closure, graph/analysis handoff, API filtering, package-supplied catalog activation, or validation acceptance may treat the token as selected. `domain.md` defines no token grammar beyond routing and must not infer token meaning from table paths, feed categories, coverage-domain values, package names, package labels, module names, OCSF classes, source-extension fields, source-native products, private route names, private vendor/product bindings, or private source inventories.
...
Correction snapshot behavior is owned by `080.CorrectionSnapshotRefPolicy`. Domain routing is resolved, but promotion remains blocked while any required correction, snapshot, assertion-transition, event-sequence, mutation-prohibition, fixture checksum, or expected checksum row is TODO-bearing or blocked.

Replay equivalence behavior is owned by `080.ReplayEquivalencePolicy`, with output-class field selection imported from owner specs. Domain routing is resolved, but promotion remains blocked until `080.ReplayEquivalencePolicyOutputClassRow` and `120-REPLAY-OUTPUT-CLASS-*` pass for every active output class.
...
Gold predicate catalog behavior is owned by `080.GoldFactPredicateContractRow` and `080.MVPGoldFactPredicateContractRowSetClosure`. Production activation remains blocked until the active MVP predicate inventory, deterministic block rows, structured schema refs, source-authority handoffs, package-set refs when package-supplied, `030.VersionManifest` refs, generated error registry rows, and `120-GOLD-PREDICATE-CATALOG-*` fixtures pass.
...
`domain_closure_state` for `DOM-TODO-020` may be changed to `resolved_owner_routed` only when `000.RunLockClosurePromotionRefSet`, `030.RunLockAcceptanceDependencyMap`, and `120.RunLockClosureValidationRows` all report run-lock closure passing for the same `SpecSetVersion`. `domain.md` must not copy lock defaults, lock algorithms, error-code tables, fixture envelope fields, expected checksum values, or validation harness behavior.
...
Package vocabulary closure is now split into two owner-routed states. `100.PackageType` token identity is closed in core. `100.PackageActivationRowFamilyClosureTable`, `100.PackageTypePolicyRowSet`, `100.PackageDeprecationWindowPolicyRow`, and `100.ProductionPackageSetManifest` close owner-local row-family interfaces. Concrete package policy, deprecation, trust, compatibility, repository evidence, release manifest, package-set manifest, validation fixture, package-set ref, generated error registry, and `030.VersionManifest` supporting material remains blocked until registered and validated. `domain.md` must not define package field schemas, activation algorithms, defaults, failure precedence, validation rows, or runtime side effects.

`090.GraphBackendSelectionPolicy` owns default backend selection. A backend selection token, including `mvp-postgresql-relational-graph.v1`, or `mvp-postgresql-age.v1` is not domain truth and is not production activation. PostgreSQL and AGE names are not domain concepts, graph semantic authority, identity authority, package activation authority, or future-provider exclusions. Production activation remains blocked until the selected `090` backend profile, `100` package-set gates, and `120` backend/package validation rows pass.
...
| `DOM-PACKAGE-TERMS-AC-001` | `domain.md` routes `package_type`, `package_type_policy`, `PackageActivationRowFamilyClosureTable`, `PackageTypePolicyRowSet`, `PackageDeprecationWindowPolicyRow`, `ProductionPackageSetManifest`, `last_known_good_package_set`, `package_quarantine`, and `emergency_package_override` to `100`; rejects broad labels, grouping bundles, package names, module names, version strings, and repository paths as vocabulary substitutes; and defines no runtime schema, default, algorithm, validation row, failure precedence, or side effect for them. |
