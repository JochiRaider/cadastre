...
| `provider_native_query_text` | SQL, Cypher, SQL/Cypher, Gremlin, AQL, or backend-native query material. | User-facing output, telemetry payload, audit detail, or production behavior without translation profile, redaction, authorization, and mutation-proof validation. | `090`, `110`, `130`, `140` | `090.GraphQueryTranslationProfile`; `110.RedactionPolicy`; `130.ValidateAnalysisQueryImport`; `140.TelemetryRedactionPolicy` | Vocabulary only; runtime remains owner-gated. |
...
| `DOM-TODO-013` | `090.GraphBackendSelectionPolicy` | graph backend default selection plus production activation gating; current omitted-backend candidate is owner-routed to `mvp-postgresql-relational-graph.v1` | `blocked_owner_catalogs` | routed through `DOM-TODO-ACTIVATION-CATALOG-CLOSURE`; `090`, `100`, and `120` own PostgreSQL default rows, optional AGE rows, explicit non-default JanusGraph rows when selected, backend profile rows, provider capability rows, package-set gates, and validation rows | validation fails | `120-DOMAIN-SECTION25-*`; `120-GRAPH-BACKEND-SELECTION-*`; `120-GRAPH-POSTGRES-*`; `120-GRAPH-AGE-*`; `120-GRAPH-BACKEND-PACKAGE-GATE-*` |
