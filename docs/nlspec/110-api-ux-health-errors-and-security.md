...
| `KNOWN_TIME_RECONSTRUCTION_REJECTED` | `080` | error | no, until evidence or policy changes | source-known-time evidence redacted | reject reconstructed known time | policy ref, missing evidence refs | `val-080-event-known-time-total` |
| `REPLAY_MUTABLE_REF` | `080` | error | no, until immutable ref supplied | mutable ref redacted | block replay before output | ref class, output class | `val-080-event-replay-total` |
| `REPLAY_INPUT_INSUFFICIENT_GRAPH_REBUILD` | `080`, `090` | blocked | retry after refs exist | graph refs redacted by policy | block graph rebuild replay | missing graph rebuild refs | `val-080-event-graph-handoff-total` |
...
| `GOLD_FACT_STRUCTURED_SCHEMA_UNKNOWN_FIELD` | `080` | error | no, until schema or candidate changes | offending field path redacted | block before fact ID computation | schema ref, field path | `val-080-structured-schema-unknown-field` |
| `GOLD_FACT_STRUCTURED_SCHEMA_MISSING_REQUIRED_FIELD` | `080` | error | no, until candidate changes | field path visible when public | block before fact ID computation | schema ref, field path | `val-080-structured-schema-missing-required` |
| `GOLD_FACT_STRUCTURED_SCHEMA_INVALID_ENUM` | `080` | error | no, until candidate or enum row changes | enum raw value redacted | block before fact ID computation or map only as declared | schema ref, field path, enum policy | `val-080-structured-schema-unknown-enum` |
| `GOLD_FACT_STRUCTURED_SCHEMA_FIELD_INVALID` | `080` | error | no, until candidate or schema row changes | invalid value redacted | block before fact ID computation | schema ref, field path, field type | `val-080-structured-schema-string-bounds` |
| `GOLD_FACT_STRUCTURED_SCHEMA_PRIVATE_BINDING_LEAK` | `080`, `110` | blocked | no, until row or candidate is redacted | private binding redacted | block before output and audit only redacted context | schema ref, redaction ref | `val-080-structured-schema-private-binding-leak` |
