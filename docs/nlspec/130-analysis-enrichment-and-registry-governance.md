...
| `feed_category_closure_row_ref` | Required when the rule uses absence-sensitive or source-effect behavior | null only when no source-effect behavior is requested | Exact selected `020.LakehouseFeedCategoryClosureRow` ref or deterministic category/effect block row ref. |
| `feed_category_closure_row_checksum` | Required when `feed_category_closure_row_ref` is required | none | Checksum of the selected feed-category closure row. |
| `effect_map_checksum` | Required when `feed_category_closure_row_ref` is required | none | Checksum of the selected feed-category effect map; missing checksum blocks analysis and derived-edge output. |
| `required_source_effect_closure_tuple_checksum` | Required when source-effect behavior is requested | none | Checksum over the expanded `000`/`060` source-effect tuple key. |
...
A derived-edge or analysis rule whose support is absence-sensitive, cleanup-sensitive, expiry-sensitive, retraction-sensitive, no-change, or watermark-affecting must include selected `020.SourceDatasetCatalogRow` refs or deterministic source-dataset block refs, selected `020.LakehouseFeedCategoryClosureRow` refs/checksums or deterministic category/effect block refs, effect-map checksum, selected `060.SourceAuthorityClosureMatrixRow` refs or deterministic source-effect block refs, expanded source-effect tuple checksum, `060.AbsenceDerivationResult` refs for the requested effect, mutation-prohibition refs for blocked/no-op cases, package-set refs when package-supplied, and `030.VersionManifest` refs before producing findings, metrics, pathfinding-visible output, gold facts through `080`, graph deltas through `090`, or any caller-visible analysis output.
